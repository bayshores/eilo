...
import { CATALOG, MODES, createDefaultState, normalizeState, previewMove, projectLayout, updateLayout, withConversationDock } from './layout.js';
...
test('withConversationDock removes only Conversation metadata and fills the untouched default slot', () => {
  ...
  assert.equal(docked.widgets.some(widget => widget.type === 'conversation'), false);
  assert.equal(docked.positions.wide.some(position => position.id === 'conversation-1'), false);
...
test('withConversationDock preserves custom arrangements, footprints, and every non-Conversation ID', () => {
  let state = updateLayout(createDefaultState(), { type: 'resizeTo', id: 'progress-1', w: 6, h: 5, mode: 'wide' });
  state = updateLayout(state, { type: 'move', id: 'progress-1', x: 0, y: 8, mode: 'wide' });
  state = updateLayout(state, { type: 'add', id: 'notes-1', widgetType: 'notes', size: 'medium', mode: 'compact' });
  const before = JSON.parse(JSON.stringify(state));
  const docked = withConversationDock(state);
  assert.equal(JSON.stringify(state), JSON.stringify(before));
  assert.equal(docked.widgets.some(widget => widget.id === 'conversation-1'), false);
  assert.deepEqual(docked.widgets.find(widget => widget.id === 'progress-1').footprints.wide, { w: 6, h: 5 });
  assert.deepEqual(docked.positions.wide.find(position => position.id === 'progress-1'), before.positions.wide.find(position => position.id === 'progress-1'));
...
test('withConversationDock is idempotent and standalone defaults remain unchanged', () => {
  ...
  assert.deepEqual(createDefaultState(), {
    version: 1,
    widgets: [{ id: 'today-1', type: 'today', size: 'large' }, { id: 'goals-1', type: 'goals', size: 'large' }, { id: 'progress-1', type: 'progress', size: 'small' }, { id: 'conversation-1', type: 'conversation', size: 'medium' }],
    positions: { wide: [{ id: 'today-1', x: 0, y: 0 }, { id: 'goals-1', x: 4, y: 0 }, { id: 'progress-1', x: 4, y: 2 }, { id: 'conversation-1', x: 7, y: 2 }] },
  });
});
...
test('previewMove swaps a displaced card into the vacated source slot without mutating input', () => {
  const state = normalizeState({ version: 1, widgets: [
    { id: 'a', type: 'progress', size: 'small' }, { id: 'b', type: 'progress', size: 'small' },
  ], positions: { wide: [{ id: 'a', x: 0, y: 0 }, { id: 'b', x: 3, y: 0 }] } }), frozen = JSON.stringify(state);
...
test('previewMove preserves heterogeneous footprints and unaffected anchors when possible', () => {
  ...
  const before = new Map(projectLayout(state).map(item => [item.id, [item.x, item.y, item.w, item.h]]));
  const after = new Map(projectLayout(preview).map(item => [item.id, [item.x, item.y, item.w, item.h]]));
  assert.deepEqual(after.get('today-1'), before.get('today-1'));
  assert.deepEqual(after.get('goals-1'), before.get('goals-1'));
...
test('previewMove returns the original for origin, invalid requests, and impossible visible placement', () => {
  const state = createDefaultState();
  assert.strictEqual(previewMove(state, { id: 'today-1', x: 0, y: 0, mode: 'wide', rows: 4 }), state);
  assert.strictEqual(previewMove(state, { id: 'missing', x: 1, y: 1, mode: 'wide', rows: 4 }), state);
  assert.strictEqual(previewMove(state, { id: 'today-1', x: NaN, y: 1, mode: 'wide', rows: 4 }), state);
  assert.strictEqual(previewMove(state, { id: 'today-1', x: 1, y: 1, mode: 'wide', rows: 3 }), state);
});

test('previewMove reserves boundary-straddling hidden cards and leaves other modes untouched', () => {
  const state = normalizeState({ version: 1, widgets: [
    { id: 'a', type: 'progress', size: 'small' }, { id: 'hidden', type: 'notes', size: 'small' },
  ], positions: { wide: [{ id: 'a', x: 0, y: 0 }, { id: 'hidden', x: 3, y: 3 }], compact: [{ id: 'a', x: 0, y: 0 }] } });
  ...
  assert.deepEqual(first.positions.wide.find(item => item.id === 'hidden'), { id: 'hidden', x: 3, y: 3 });
  assert.deepEqual([projectLayout(first, 'wide').find(item => item.id === 'a').x, projectLayout(first, 'wide').find(item => item.id === 'a').y], [3, 1]);
  assert.deepEqual([projectLayout(first, 'wide').find(item => item.id === 'hidden').x, projectLayout(first, 'wide').find(item => item.id === 'hidden').y], [3, 3]);
  assertUsable(first, 'wide');
});

test('previewMove snaps a heterogeneous collision near its target while retaining untouched peers', () => {
  const state = normalizeState({ version: 1, widgets: [
    { id: 'target', type: 'progress', size: 'small' }, { id: 'large', type: 'goals', size: 'large' }, { id: 'fixed', type: 'progress', size: 'small' },
  ], positions: { wide: [{ id: 'target', x: 0, y: 2 }, { id: 'large', x: 3, y: 0 }, { id: 'fixed', x: 0, y: 0 }] } });
...
test('previewMove remains bounded on a dense visible fixture', () => {
  const widgets = Array.from({ length: 12 }, (_, index) => ({ id: `p-${index}`, type: 'progress', size: 'small' }));
  const positions = widgets.map((widget, index) => ({ id: widget.id, x: (index % 4) * 3, y: Math.floor(index / 4) * 2 }));
...
test('previewMove bounded fallback keeps an exact dense progress target instead of returning origin', () => {
  const preview = previewMove(createDefaultState(), { id: 'progress-1', x: 0, y: 2, mode: 'wide', rows: 4 });
  const layout = new Map(projectLayout(preview).map(item => [item.id, item]));
...
test('previewMove bounded fallback keeps an exact dense conversation target', () => {
  const preview = previewMove(createDefaultState(), { id: 'conversation-1', x: 0, y: 2, mode: 'wide', rows: 4 });
  const layout = new Map(projectLayout(preview).map(item => [item.id, item]));
  assert.deepEqual([layout.get('goals-1').x, layout.get('goals-1').y], [0, 0]);
  assert.deepEqual([layout.get('today-1').x, layout.get('today-1').y], [8, 0]);
...
test('layout transfer carries normalized view metadata and presentation preferences only', async () => {
  ...
  const original = createDefaultState(); original.messages = ['private']; original.widgets[0].content = 'private note';
  const result = readLayoutTransfer(layoutTransferHash(original, {pin:true,reducedMotion:true,name:'private name'}));
...
test('malformed or oversized layout transfer leaves stored preferences alone', async () => {
  ...
  for (const value of ['', '#other=value', '#home-layout=%xx', '#home-layout=' + encodeURIComponent('{}'), '#home-layout=' + 'a'.repeat(33000)]) assert.equal(readLayoutTransfer(value),null);
});
