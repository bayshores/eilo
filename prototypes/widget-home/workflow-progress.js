const RUN_STATUSES = new Set(['running', 'completed', 'partial', 'failed', 'cancelled', 'interrupted']);
...
const permittedSourceUrl = value => {
  try {
    ...
    return url.protocol === 'https:' && (url.origin === 'https://mail.google.com' || url.origin === 'https://calendar.google.com') ? url.href : '';
  } catch { return ''; }
};
...
export function normalizeWorkflowRun(run = {}) {
  ...
  return {
    id: String(run.id || ''),
    status,
    title: typeof run.title === 'string' && run.title.trim() ? run.title.trim() : 'Preparing your briefing',
    ...
    sources: Array.isArray(run.sources) ? run.sources.map((source, index) => ({
      id: String(source?.id || `source-${index}`),
      label: typeof source?.label === 'string' && source.label.trim() ? source.label.trim() : 'Open source',
...
export function workflowTransition(previous, next) {
  ...
  return {
    ...
    enteredTerminal: Boolean(before && before.status !== after.status && terminal.has(after.status)),
...
export function mountWorkflowProgress(container, { onCancel = () => {}, onConnect = () => {}, onDismiss = () => {}, onOpenSource = () => false } = {}) {
  if (!(container instanceof Element)) throw new TypeError('mountWorkflowProgress requires a DOM element container.');
  ...
  detailToggle.type = 'button'; detailToggle.setAttribute('aria-controls', details.id);
  const cancel = node('button', 'workflow-progress__button workflow-progress__button--quiet', 'Stop briefing');
  cancel.type = 'button';
  const connect = node('button', 'workflow-progress__button workflow-progress__button--primary', 'Connect a source');
  connect.type = 'button';
  const dismiss = node('button', 'workflow-progress__button workflow-progress__button--quiet', 'Dismiss');
  dismiss.type = 'button';
  ...
  root.addEventListener('keydown', event => {
    if (event.key === 'Escape' && expanded) { event.preventDefault(); setExpanded(false); detailToggle.focus(); }
  });

  function render(nextRun) {
    if (destroyed) return;
    const priorFocus = document.activeElement === detailToggle ? 'details'
      : document.activeElement === cancel ? 'cancel'
        : document.activeElement === dismiss ? 'dismiss' : '';
    ...
    current.textContent = running ? running.label : (run.summary || (run.steps.length ? 'No step is currently running.' : 'No source checks have started.'));
    ...
    if (run.sources_checked !== null) facts.append(node('span', '', `${run.sources_checked} source${run.sources_checked === 1 ? '' : 's'} checked`));
    facts.hidden = !facts.childElementCount;
    list.replaceChildren();
    run.steps.forEach(step => {
      const item = node('li', `workflow-progress__step is-${step.status}`);
      const marker = node('span', 'workflow-progress__marker'); marker.setAttribute('aria-hidden', 'true');
    ...
    const safeSources = run.sources.map(source => ({ ...source, safeUrl: permittedSourceUrl(source.url) })).filter(source => source.safeUrl);
    if (safeSources.length) {
      ...
      safeSources.forEach(source => {
        ...
        link.href = source.safeUrl; link.target = '_blank'; link.rel = 'noreferrer noopener';
        link.addEventListener('click', event => {
          if (link.dataset.allowFallback === 'true') { delete link.dataset.allowFallback; return; }
          event.preventDefault();
          Promise.resolve(onOpenSource(run, source)).then(opened => {
            if (opened === false && link.isConnected) { link.dataset.allowFallback = 'true'; link.click(); }
          }).catch(() => { /* Keep the source private; the native layer owns errors. */ });
        });
    ...
    connect.hidden = !run.needs_connection && (Boolean(run.steps.length) || run.status === 'running');
    ...
    if (priorFocus === 'details' && !detailToggle.hidden) detailToggle.focus({ preventScroll: true });
  ...
  setExpanded(false);
  return { update: render, destroy() { destroyed = true; root.remove(); } };
}
