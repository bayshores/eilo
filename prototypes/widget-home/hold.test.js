...
test('a short press cancelled before the threshold never activates', () => {
  const timers = fakeTimers(), activated = [], pending = [];
  const hold = createHoldGesture({ onActivate: value => activated.push(value), onPending: value => pending.push(value), ...timers });
  hold.start({ id: 'today-1', pointerId: 4, x: 10, y: 20 });
  timers.tick(419); hold.cancel(); timers.tick(1000);
...
test('a hold activates exactly once at the threshold with its original candidate', () => {
  const timers = fakeTimers(), activated = [], pending = [];
  const hold = createHoldGesture({ delay: 50, onActivate: value => activated.push(value), onPending: value => pending.push(value), ...timers });
  hold.start({ id: 'notes-1', pointerId: 7, x: 1, y: 2 });
  timers.tick(49); assert.equal(activated.length, 0);
  timers.tick(1); timers.tick(500);
...
test('scroll-like movement beyond tolerance cancels the pending hold', () => {
  const timers = fakeTimers(), activated = [], pending = [];
  const hold = createHoldGesture({ tolerance: 8, onActivate: value => activated.push(value), onPending: value => pending.push(value), ...timers });
...
test('replacement and a mismatched pointer each cancel the previous candidate', () => {
  ...
  timers.tick(1000);
  assert.deepEqual(activated, []);
  assert.deepEqual(pending.map(value => value?.id ?? null), ['today-1', null, 'goals-1', null]);
});
