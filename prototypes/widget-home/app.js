import { CATALOG, MODES, createDefaultState, normalizeState, projectLayout, updateLayout, previewMove, resizeWithinHome, fitWithinHome, layoutTransferHash, readLayoutTransfer, withConversationDock } from './layout.js';
...
const createLiveHome = document.documentElement.dataset.source === 'live'
  ? (await import('./home-live.js')).createLiveHome : null;
...
function finishLandings() { for (const finish of [...landings]) finish(); }
...
function clearGestureFrame() { cancelAnimationFrame(gestureFrame); gestureFrame = 0; }
...
const transferred = document.documentElement.dataset.source === 'live' ? readLayoutTransfer(location.hash) : null;
let state = normalizeState(transferred?.layout ?? readStorage(KEY, createDefaultState()));
...
const storedPrefs = { ...readStorage(PREFS_KEY, {}), ...transferred?.preferences };
...
const live = document.documentElement.dataset.source === 'live' ? createLiveHome({
  openDetail: showDetail, dialog: detail,
  onViewChange: () => { if (editing) setEditing(false); closeAccount(); closeWidgetMenu(); requestAnimationFrame(renderBoard); },
  refreshWidgets: () => {
    for (const element of elements.values()) {
      if (!['today','goals','progress','conversation'].includes(element.dataset.type)) continue;
      const body = element.querySelector('.widget-content');
      const focused = body.contains(document.activeElement) ? document.activeElement.textContent : null;
      ...
      if (focused) [...body.querySelectorAll('button')].find(button => button.textContent === focused)?.focus();
    }
  },
}) : null;
...
function commit(action, message) {
  ...
  const next = action.type === 'resizeTo' ? resizeToHome(state,action.id,action.w,action.h) : action.type === 'move' ? previewMove(state,{...action,mode,rows:rowBudget}) : updateLayout(state, { ...action, mode });
...
function placeOnHome(base,id) {
  ...
  const next=item.h>rowBudget ? updateLayout(base,{type:'resizeTo',id,w:item.w,h:rowBudget,mode}) : base;
...
function createWidget(widget) {
  ...
  const cue = document.createElement('span'); cue.className = 'hold-cue'; cue.setAttribute('aria-hidden','true');
  cue.style.setProperty('--hold-step', WIDGET_HOLD_MS / 6 + 'ms');
  cue.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="8" cy="5" r="1.7"/><circle cx="16" cy="5" r="1.7"/><circle cx="8" cy="12" r="1.7"/><circle cx="16" cy="12" r="1.7"/><circle cx="8" cy="19" r="1.7"/><circle cx="16" cy="19" r="1.7"/></svg><span class="hold-cue-label">Hold to move</span>';
  cue.querySelectorAll('circle').forEach((circle,index) => circle.style.setProperty('--dot',index));
  element.append(cue);
  ...
  const limit = document.createElement('span'); limit.className='resize-limit-note';limit.textContent='No more room';limit.hidden=true;limit.setAttribute('aria-hidden','true');element.append(limit);
  ...
  grip.addEventListener('blur', () => { if (keyboardMove?.id === widget.id) finishKeyboardMove(false); });
...
function updateGeometry() {
  ...
  rowBudget = mode === 'stacked' ? 2*Math.max(1,Math.floor((scroller.clientHeight+gap)/(244+2*gap))) : scroller.clientHeight >= (live ? (document.querySelector('.conversation-dock[data-open="true"], .conversation-dock .live-update:not([hidden])') ? 300 : 380) : 500) ? 4 : 2;
...
function renderBoard() {
  ...
  if (!initialFitChecked) {
    ...
    if(repaired!==state){state=repaired;saveLayout();}
  }
  const layout = projectLayout(drag?.preview ?? resizeDrag?.preview ?? keyboardMove?.preview ?? state,mode), ids = new Set(layout.map(item => item.id));
...
function setEditing(value) {
  ...
  finishLandings(); editing = value; closeWidgetMenu();
...
$('.app-window').addEventListener('pointerdown', event => {
  if (!editing || event.button !== 0 || !event.isPrimary || drag || resizeDrag || gallery.open || detail.open) return;
  ...
  if (event.target.matches('.app-window,.workspace,.board-scroll,.board,.home-header,.header-actions,.rail-zone,.empty-home')) {
    setEditing(false);
  }
});
...
function renderGallery() {
  ...
  const types = Object.keys(CATALOG).filter(type => (!live || type !== 'conversation') && TITLES[type].toLowerCase().includes(query));
...
function handleMoveKeys(event,id) {
  ...
  if ([' ','Enter'].includes(event.key)) {
    ...
    closeWidgetMenu(); finishLandings();
    const before = structuredClone(state), item = projectLayout(before,mode).find(widget => widget.id === id);
    keyboardMove = {id,before,preview:before,target:{x:item.x,y:item.y},item};
    elements.get(id).classList.add('keyboard-moving'); event.currentTarget.setAttribute('aria-pressed','true');
    announce('Moving widget. Use arrow keys, Enter to place, or Escape to cancel.');
  } else if (keyboardMove?.id === id && event.key === 'Escape') {
    event.preventDefault(); event.stopPropagation(); finishKeyboardMove(false);
  } else if (keyboardMove?.id === id && ['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(event.key)) {
    event.preventDefault();
    const moving = keyboardMove;
    const x = Math.max(0,Math.min(MODES[mode]-moving.item.w,moving.target.x+(event.key==='ArrowLeft'?-1:event.key==='ArrowRight'?1:0)));
    const y = Math.max(0,Math.min(rowBudget-moving.item.h,moving.target.y+(event.key==='ArrowUp'?-1:event.key==='ArrowDown'?1:0)));
    moving.target={x,y}; moving.preview=previewMove(moving.before,{id,x,y,mode,rows:rowBudget});
    renderBoard(); const placed=projectLayout(moving.preview,mode).find(widget=>widget.id===id);
    announce(`Column ${placed.x+1}, row ${placed.y+1}.`);
  }
}
function finishKeyboardMove(keep) {
  if (!keyboardMove) return;
  const {id,before,preview} = keyboardMove; keyboardMove = null;
  const changed = preview !== before;
  if (keep && changed) { undoState=before; state=preview; saveLayout(); }
  elements.get(id)?.classList.remove('keyboard-moving'); elements.get(id)?.querySelector('.move-widget').removeAttribute('aria-pressed');
  renderBoard();
  if (keep && changed) toast('Widget moved.',true); else announce(keep?'Widget placed.':'Move cancelled.');
}
function startPointerMove(event,id) {
  if (!editing || event.button !== 0 || drag || resizeDrag) return;
  closeWidgetMenu(); finishLandings(); clearTimeout(railTimer);
  if (keyboardMove) finishKeyboardMove(true);
  const element = elements.get(id), rect = element.getBoundingClientRect();
  const before = structuredClone(state), item = projectLayout(before,mode).find(item => item.id === id);
  drag = {
    id, handle:event.currentTarget, pointerId:event.pointerId,
    startX:event.clientX, startY:event.clientY, offsetX:event.clientX-rect.left, offsetY:event.clientY-rect.top,
    clientX:event.clientX, clientY:event.clientY, started:false, before, preview:before, item, mode, rows:rowBudget,
    ghost:null, target:{x:item.x,y:item.y}, previews:new Map([[`${item.x},${item.y}`,before]])
  };
...
function liftWidget() {
  ...
  drag.started = true;
  const element = elements.get(drag.id), rect = element.getBoundingClientRect();
  const ghost = element.cloneNode(true);
  ghost.classList.add('drag-ghost'); ghost.classList.remove('keyboard-moving','holding-widget','hold-ready');
  ghost.removeAttribute('aria-labelledby'); ghost.removeAttribute('aria-describedby');
  ghost.setAttribute('aria-hidden','true'); ghost.inert = true;
  ghost.querySelectorAll('[id]').forEach(node => node.removeAttribute('id'));
  ghost.style.width = rect.width+'px'; ghost.style.height = rect.height+'px';
  ghost.style.transform = `translate3d(${rect.left}px,${rect.top}px,0)`;
  ghost.style.removeProperty('--left'); ghost.style.removeProperty('--top');
  // Keep the same editing styles and content geometry as the card being held.
  $('.app-window').append(ghost); drag.ghost = ghost;
  if (!reducedMotion()) drag.lift = ghost.animate([{scale:'1'},{scale:'1.012'}],{duration:180,easing:MOTION_EASE});
...
function startResize(event,id) {
  ...
  finishLandings(); clearTimeout(railTimer);
  const item = projectLayout(state,mode).find(widget => widget.id === id);
  const before = structuredClone(state);
  resizeDrag = {id,pointerId:event.pointerId,handle:event.currentTarget,startX:event.clientX,startY:event.clientY,
    clientX:event.clientX,clientY:event.clientY,item,before,preview:before,target:{w:item.w,h:item.h},
    stepX:cellWidth+gap,stepY:rowHeight+gap,mode};
...
function resizeWithKeys(event,id) {
  ...
  if(!commit({type:'resizeTo',id,w,h},'Widget resized.'))announce('Size limit reached. The other widgets remain on Home.');
}
function resizePosition() {
  if (!resizeDrag) return;
  const r = resizeDrag;
  const minWidth = r.mode === 'wide' ? CATALOG[r.item.type].sizes.small.w : r.mode === 'compact' ? 3 : 1;
  const w = Math.max(minWidth,snapDragCell(r.item.w+(r.clientX-r.startX)/r.stepX,r.target.w,MODES[r.mode]));
  const h = Math.max(2,snapDragCell(r.item.h+(r.clientY-r.startY)/r.stepY,r.target.h,rowBudget));
...
function finishResize(keep) {
  if (!resizeDrag) return;
  clearGestureFrame(); if (keep) resizePosition();
  ...
  const limit=elements.get(finished.id)?.querySelector('.resize-limit-note');if(limit)limit.hidden=true;
  const changed = finished.preview !== finished.before;
  if (keep && changed) { undoState=finished.before; state=finished.preview; saveLayout(); }
  renderBoard();
...
function snapDragCell(value, previous, limit) {
  ...
  const snapped = Math.abs(value-previous) < .66 ? previous : Math.round(value);
  return Math.max(0,Math.min(limit,snapped));
}
function dragPosition() {
  if (!drag?.started) return;
  const rect = board.getBoundingClientRect(), d = drag;
  const left = d.clientX-d.offsetX, top = d.clientY-d.offsetY;
  d.ghost.style.transform = `translate3d(${left}px,${top}px,0)`;
  const x = snapDragCell((left-rect.left)/(cellWidth+gap),d.target.x,MODES[d.mode]-d.item.w);
  const y = snapDragCell((top-rect.top)/(rowHeight+gap),d.target.y,d.rows-d.item.h);
  if (d.target.x === x && d.target.y === y) return;
  d.target = {x,y};
  const key = `${x},${y}`;
  if (!d.previews.has(key)) d.previews.set(key,previewMove(d.before,{id:d.id,x,y,mode:d.mode,rows:d.rows}));
...
document.addEventListener('pointermove',event => {
  ...
  if (resizeDrag && event.pointerId === resizeDrag.pointerId) {
    event.preventDefault(); resizeDrag.clientX=event.clientX; resizeDrag.clientY=event.clientY;
    queueGestureFrame(); return;
  }
  ...
  event.preventDefault(); liftWidget(); queueGestureFrame();
},{passive:false});
function landWidget(finished) {
  const element = elements.get(finished.id), ghost = finished.ghost;
  if (!ghost) return;
  const item = projectLayout(state,mode).find(item => item.id === finished.id);
  let animation = null, done = false;
  const cleanup = () => {
    if (done) return; done=true;
    animation?.cancel(); finished.lift?.cancel(); ghost.remove();
    element?.classList.remove('is-dragging'); landings.delete(cleanup);
  };
  landings.add(cleanup);
  if (reducedMotion() || !item || element.hidden) { cleanup(); return; }
  const rect = board.getBoundingClientRect(), scale = getComputedStyle(ghost).scale;
...
function finishPointerMove(keep) {
  if (!drag) return;
  clearGestureFrame(); if (keep) dragPosition();
  ...
  if (finished.started) {
    const changed = finished.preview !== finished.before;
    if (keep && changed) { undoState=finished.before; state=finished.preview; saveLayout(); }
    renderBoard(); landWidget(finished);
...
document.addEventListener('pointerup',event => {
  ...
  if (drag && event.pointerId === drag.pointerId) {
    drag.clientX=event.clientX; drag.clientY=event.clientY; finishPointerMove(true);
  }
  if (resizeDrag && event.pointerId === resizeDrag.pointerId) {
    resizeDrag.clientX=event.clientX; resizeDrag.clientY=event.clientY; finishResize(true);
  }
});
...
scroller.addEventListener('scroll',() => { cancelHeldPress(); if (drag) queueGestureFrame(); else closeWidgetMenu(); },{passive:true});
...
function setRail(open, restore = false) {
  ...
  if (railOpen !== open) {
    finishLandings(); workspace.classList.add('nav-transitioning'); clearTimeout(railTransitionTimer);
...
function scheduleRailHide() {
  clearTimeout(railTimer); if (prefs.pin || drag || resizeDrag) return;
  railTimer = setTimeout(() => { if (!drag && !resizeDrag && !railHovered && !rail.contains(document.activeElement) && !accountOpen) setRail(false); },500);
}
$('.rail-zone').addEventListener('pointerenter',event => { if (event.pointerType === 'mouse') { railHovered=true; if(!drag && !resizeDrag)setRail(true); } });
...
$$('[data-detail]').forEach(button => button.addEventListener('click',() => { const type=button.dataset.detail; if(live){live.showPage(type);return;} if(type==='home') { scroller.scrollTo({top:0,behavior:prefs.reducedMotion?'instant':'smooth'}); return; } showDetail(type); }));
...
function showDetail(type) {
  if (live && type === 'conversation') { live.focusConversation(); return; }
  if (live && ['home','goals','today','progress','activity','connections','chats','projects'].includes(type)) {
    live.showPage(type==='today'||type==='progress'?'goals':type, {goalFilter:type==='today'||type==='goals'?'open':type==='progress'?'all':undefined}); return;
  }
  ...
  detail.dataset.detail = type; detail.classList.remove('live-conversation-dialog');
  if (live?.renderDetail(type,title,body)) { detail.showModal(); return; }
  if (type === 'overflow') {
    ...
    body.querySelector('.motion-setting').addEventListener('change',event => { prefs.reducedMotion=event.target.checked; document.body.classList.toggle('reduce-motion',prefs.reducedMotion);finishLandings();writeStorage(PREFS_KEY,prefs); });
    ...
    if (!live) {
      const link = document.createElement('a'); link.className='button'; link.textContent='Use this layout in eïlo';
      link.href='http://127.0.0.1:8765/home/' + layoutTransferHash(state,prefs); body.append(link);
    }
  } else if (type === 'profile') {
    title.textContent=live?'Local profile':'Prototype profile'; body.innerHTML='<form class="profile-form"><label class="field">Display name<input type="text" maxlength="40" autocomplete="off" required></label><button class="button primary">Save name</button></form><p class="muted">This display name is saved in this browser. It does not change your sign-in account.</p>';
    body.querySelector('input').value=prefs.name;body.querySelector('form').addEventListener('submit',event => {event.preventDefault();const value=body.querySelector('input').value.trim();if(!value)return;prefs.name=value;writeStorage(PREFS_KEY,prefs);renderProfile();detail.close();toast('Display name saved.');});
...
window.addEventListener('resize',() => { cancelHeldPress();if(drag)finishPointerMove(false);if(resizeDrag)finishResize(false);if(keyboardMove)finishKeyboardMove(false);finishLandings();closeWidgetMenu();if(gallery.open)positionGallery(); });
...
window.addEventListener('pagehide',() => { cancelHeldPress(); if(drag)finishPointerMove(false);finishLandings(); if(resizeDrag)finishResize(false);if(keyboardMove)finishKeyboardMove(false);writeStorage(CONTENT_KEY,content); });
motionPreference.addEventListener('change',() => { finishLandings(); if(reducedMotion())drag?.lift?.cancel(); });
...
if (transferred) { writeStorage(KEY,state); writeStorage(PREFS_KEY,prefs); history.replaceState(null,'',location.pathname+location.search); }
live?.start();
