...
The study reads the current widget prototype source and renders it inside a comparison frame. Its separate loopback origin gives it independent browser storage. A/B/C change typography only; Previous restores the earlier system-sans treatment. Sean selected B (IBM Plex Sans) on September 9, 2026; it is now applied in the main prototype. The study keeps the earlier HTML/CSS baseline for comparison.
