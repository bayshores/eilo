...
def _task_state(value: Any) -> dict[str, Any]:
    ...
    if (
        not isinstance(value["break_active"], bool)
    ...
    for raw in value["tasks"]:
        ...
        if target is not None and (
            isinstance(target, bool) or not isinstance(target, int) or not 1 <= target <= 10000
        ):
            raise InputError("invalid task")
        ...
        if (
            ...
            or not 0 <= completed <= 10000
            or (target is not None and completed > target)
        ):
            raise InputError("invalid task")
...
def validate_input(value: Any) -> dict[str, Any]:
    ...
    if not isinstance(observation, dict):
        raise InputError("invalid observation")
    coarse = set(observation) == {"kind"} and observation["kind"] in (
        "activity_unshared",
        "activity_unknown",
    )
    detailed = (
...
def _parse_decision(text: Any, event_id: str) -> dict[str, str]:
    ...
    return (
        ...
        else {"event_id": event_id, "decision": "quiet", "message": ""}
    )
...
def dry_audit() -> dict[str, Any]:
    ...
    return {
        ...
        "tool_schema_count": audit["tool_schema_count"],
    }
    ...
    with SessionDB(read_only=True) as db:
        ...
        if on_preview is not None:
            ...
            preview = JsonTextPreview(
                "message",
                required={"event_id": event["event_id"]},
                allowed={"decision": {"ask", "check_in"}},
                max_chars=400,
            )
