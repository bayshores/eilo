...
def initial_state() -> dict:
    return {"allowed_hosts": list(DEFAULT_HOSTS), "human_epoch": 0,
            "last_human_at": 0, "last_decision_at": 0, "call_times": [],
            "events": {}, "seen_events": [], "last_fingerprint": None}
...
def canonical_goal_message(action: str, text: str = "") -> str:
    if action == "set" and isinstance(text, str) and 0 < len(text.strip()) <= 500 and "\n" not in text:
        value = text.strip().rstrip('.!? ')
        if value:
            return f"Set my goal to {value}"
    messages = {"break": "I'm taking a break.", "resume": "Resume my goal.", "cancel": "Cancel my goal."}
...
def normalize_host(host: str) -> str:
    ...
    if not re.fullmatch(r"(?=.{3,253}$)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}", host):
...
def sanitize_observation(raw: dict, allowed_hosts: list[str]) -> dict:
    ...
    if isinstance(raw, dict) and set(raw) == {"kind"} and raw["kind"] in ("activity_unshared", "activity_unknown"):
        return dict(raw)
    if not isinstance(raw, dict) or set(raw) != {"kind", "origin", "title"} or raw.get("kind") != "approved_study_context":
        return invalid
    ...
    return {"kind": "approved_study_context", "origin": f"https://{parsed.hostname}", "title": title}
...
def admit(state: dict, observation: dict, *, now: float, stable_since: float,
          lease_active: bool, human_busy: bool, has_open_work: bool) -> tuple[bool, str]:
    if not lease_active or human_busy or not has_open_work:
        return False, "inactive"
    if observation.get("kind") not in ("approved_study_context", "activity_unshared", "activity_unknown"):
