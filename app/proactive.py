...
class ProactiveLoop:
    ...
    def active(self):
        return (
            self.mode == "active" and self.client_id is not None and self.now() < self.lease_until
        )
    ...
    def snapshot(self):
        ...
        return {
            ...
            "activity": {
                ...
                "lease_client_id": self.client_id if self.active() else None,
    ...
    def _recent_events(self):
        ...
        for event in self.state.get("events", {}).values():
            ...
            events.append(item)
        return sorted(events, key=lambda item: item.get("created_at", float("-inf")), reverse=True)[
            :30
        ]
    ...
    def _eligible_at(self, phase, now):
        ...
        times = [
            stamp
            for stamp in (self._timestamp(value) for value in self.state["call_times"])
    ...
    def check_in_status(self):
        ...
        return {
            ...
            "enabled": enabled,
            "phase": phase,
            "eligible_at": self._eligible_at(phase, now),
            "evaluated_at": now,
            "last_observation": last_observation,
            "rules": {
                "stable_seconds": STABLE_SECONDS,
                "human_grace_seconds": HUMAN_GRACE_SECONDS,
                "cooldown_seconds": DECISION_COOLDOWN_SECONDS,
                "max_per_hour": MAX_CALLS_HOUR,
    ...
    def control(self, action, client_id):
        if action == "enable_check_ins":
            ...
            if not self.helper.is_file():
                raise ValueError(
                    "The local foreground helper is not built yet. Conversation and goal controls still work."
                )
            if self.active() and client_id != self.client_id:
                raise ValueError(
                    "Another open page owns activity collection. Pause it before enabling this page."
                )
            self.invalidate()
            ...
            if not self.collector or self.collector.done():
                self.collector = asyncio.create_task(
                    self.collect(), name="consented-open-page-activity"
                )
        elif action in ("pause", "off"):
            # Any local page can make the stop effective; only the owner may extend consent.
            self.stop(
                "paused" if action == "pause" else "off",
                "Activity is paused." if action == "pause" else "Activity is off.",
            )
        else:
    ...
    def renew(self, client_id):
        if not self.active() or client_id != self.client_id:
            raise ValueError(
                "Activity lease ended. Enable it explicitly in this page to start again."
            )
    ...
    def arm_lease_timer(self):
        ...
        def expire():
            ...
            self.stop(
                "paused",
                "The open-page lease expired. Activity stopped; enable it explicitly to resume.",
            )
            self.chat.changed()

        self.lease_timer = asyncio.get_running_loop().call_later(
            max(0, self.lease_until - self.now()), expire
        )
    ...
    def stop(self, mode="off", reason="Activity is off."):
        ...
        if (
            self.collector
            and not self.collector.done()
            and self.collector is not asyncio.current_task()
        ):
            ...
            while self.active():
                ...
                if now - last_sample >= SAMPLE_SECONDS:
                    ...
                    if is_chrome is True:
                        ...
                        self.reason = (
                            "The foreground sample is unavailable. No background decision was made."
                        )
                        self.invalidate()
                    self.chat.changed()
    ...
    def maybe_decide(self, now=None):
        ...
        event = {
            ...
            "human_epoch": self.state["human_epoch"],
        ...
        self.event_task = asyncio.create_task(
            self.decide(payload), name="native-accountability-decision"
        )
        ...
        try:
            ...
            async with self.chat.native_lock:
                ...
                if decision:
                    allowed_ids = {
                        task["id"]
                        for task in self.chat.meta["tasks"]["tasks"]
                        if task["status"] == "open"
                    }
                    if any(
                        identity not in allowed_ids
                ...
                if decision and self.current(event) and event.get("observed_session_id"):
                    self.ledger.associate(
                        event["observed_session_id"],
                        decision.get("related_task_ids", []),
                        event["task_revision"],
                    )
            ...
            if (
                self.stream
                and self.stream.get("id") == event_id
                and self.stream.get("status") == "writing"
            ):
                self.stream = {**self.stream, "status": "interrupted"}
            self.event_id = None
            self.publishing = False
