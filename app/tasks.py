...
def apply_operations(
    ...
    *,
    ...
    if type(based_on_revision) is not int or based_on_revision != state["revision"]:
        raise TaskConflict(
            "Your tasks changed while this request was being prepared. Review the current list and try again."
        )
    ...
    for operation in operations:
        ...
        if name == "add":
            ...
            if (
                ...
                or not re.fullmatch(r"new_[A-Za-z0-9_]{1,40}", temp_id)
                or temp_id in aliases
            ):
            ...
            task = {
                ...
                "completed_count": 0,
                "created_at": now,
                "created_source": {**source, "request_id": request_id},
            }
            ...
            acknowledgments.append(
                f"Added “{task['title']}”"
                + (f" ({task['due_text']})" if task["due_text"] else "")
                + "."
            )
        elif name == "edit":
            ...
            if task["target_count"] is not None and task["completed_count"] > task["target_count"]:
                raise TaskError(
                    "The target cannot be smaller than recorded progress. Correct progress first."
                )
            provenance(task)
            ...
            acknowledgments.append(
                f"Current focus: “{task['title']}”." if task else "Cleared the current focus."
            )
        elif name in ("complete", "cancel", "reopen"):
            _shape(operation, ("op", "task_id"))
            task = resolve(operation["task_id"])
            require_available(task)
            task["status"] = {"complete": "completed", "cancel": "cancelled", "reopen": "open"}[
                name
            ]
            ...
            provenance(task)
            acknowledgments.append(
                f"{ {'complete': 'Completed', 'cancel': 'Cancelled', 'reopen': 'Reopened'}[name] } “{task['title']}”."
            )
        elif name == "delete":
            _shape(operation, ("op", "task_id"))
            task = resolve(operation["task_id"])
            ...
            if (
                task["status"] != "open"
                or task["target_count"] is None
                or count > task["target_count"]
            ):
                raise TaskError(
                    "Progress needs an open task and cannot exceed its target quantity."
                )
            task["completed_count"] = count
            provenance(task)
            acknowledgments.append(
                f"Recorded {count} of {task['target_count']} {task['unit'] or 'items'} for “{task['title']}”."
            )
        elif name == "break":
            ...
            acknowledgments.append(
                "Break started. Your tasks are still here."
                if operation["active"]
                else "Break ended."
            )
        else:
    ...
    updated["revision"] += 1
    updated["applied_requests"][request_id] = {
...
def bind_model_proposal(raw, *, request_id, revision):
    ...
    if not isinstance(value, dict) or set(value) not in (
        fields,
        fields | {"request_id", "based_on_revision"},
    ):
        return None
    if (
        not isinstance(value.get("kind"), str)
        or value["kind"] not in {"update", "chat", "clarify"}
...
def validate_proposal(value, *, request_id, revision):
    ...
    if (
        ...
        or type(value["based_on_revision"]) is not int
        or value["based_on_revision"] != revision
    ):
        raise TaskConflict(
            "The proposed change no longer matches this request or the current task list."
        )
    if value["kind"] not in ("update", "chat", "clarify") or not isinstance(
    ...
    if (
        ...
        or (value["kind"] != "update" and not reply.strip())
        or len(reply) > 8000
...
def migrate_legacy(goal: dict | None) -> dict:
    ...
    state, _ = apply_operations(
        ...
        based_on_revision=0,
        request_id="legacy-single-goal-v1",
        source={
            "kind": "migration",
            "legacy_request_id": goal.get("source_request_id"),
            "legacy_version": goal.get("version", 0),
        },
    )
    return state
