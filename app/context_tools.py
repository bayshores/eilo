...
def validate_bridge(value):
    if not isinstance(value,dict) or set(value)!= {'url','token'}: raise ValueError('invalid bridge')
    url=urlsplit(value['url'])
    if (url.scheme!='http' or url.hostname!='127.0.0.1' or not url.port or not 1024<=url.port<=65535
            or url.path!='/read' or url.query or url.fragment or url.username or url.password
...
class ReadTools:
    ...
    def handler(self,name,args):
        ...
        try:
            request=urllib.request.Request(self.capability['url'],data=payload,method='POST',
                headers={'Authorization':'Bearer '+self.capability['token'],'Content-Type':'application/json'})
            ...
            if len(raw)>100000: raise ValueError('too large')
            ...
            if data is not None:
                ...
                if not isinstance(source_id,str) or not re.fullmatch(r'S[0-9]{1,3}',source_id): raise ValueError('invalid source')
                encoded=json.dumps(data,ensure_ascii=False)
                if self.size+len(encoded)>65000: raise ValueError('limit')
            ...
            safe={key:receipt[key] for key in ('status','source_id','detail') if key in receipt}
            return json.dumps(safe)
        except Exception:
            return json.dumps({'status':'unavailable','detail':'The source read did not complete. Do not claim it was checked.'})

    def register(self):
        from tools.registry import registry
        schemas=[
            ('eilo_sources','Read which inboxes and calendars the user has enabled for this request.',{},[]),
            ('eilo_search_mail','Search one enabled inbox; returns thread IDs. Read threads before making claims.',
                {'account_id':{'type':'string'},'query':{'type':'string'},'days':{'type':'integer','minimum':1,'maximum':90}},['account_id']),
            ('eilo_read_mail','Read one thread discovered by eilo_search_mail this turn. Text only; no attachments.',
                {'account_id':{'type':'string'},'thread_id':{'type':'string'}},['account_id','thread_id']),
            ('eilo_read_calendar','Read events on selected calendars in the next 30 days only if enabled for answers.',{},[])]
        for name,description,properties,required in schemas:
            registry.register(name=name,toolset='eilo_context',schema={'name':name,'description':description,
                'parameters':{'type':'object','properties':properties,'required':required,'additionalProperties':False}},
                handler=lambda args,_name=name,**kwargs:self.handler(_name,args),emoji='')
    ...
    def close(self):
        self.sources.clear()
        if self.agent: self.agent.ephemeral_system_prompt=self.base_policy
