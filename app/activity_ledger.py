...
class ActivityLedger:
    ...
    def __init__(self, state: dict, now=time.time, id_factory=lambda: uuid.uuid4().hex):
        ...
        journal["active"] = (
            journal.get("active") if isinstance(journal.get("active"), str) else None
        )
        journal["manual_revision"] = (
            journal.get("manual_revision") if self._revision(journal.get("manual_revision")) else 0
        )
        journal["control_receipts"] = (
            journal.get("control_receipts")
            if isinstance(journal.get("control_receipts"), dict)
            else {}
        )
    ...
    def _active(self):
        ...
        return next(
            (
                ...
                and session.get("end") is None
                and "trashed_at" not in session
            ),
            None,
        )
    ...
    def _trim(self, now):
        ...
        self.journal["sessions"] = [
            session
            for session in self._sessions()
            if isinstance(session, dict) and session.get("last_seen", 0) >= cutoff
        ][-MAX_SESSIONS:]
    ...
    def associate(self, session_id, task_ids, task_revision):
        if (
            ...
            or len(task_ids) > MAX_RELATED_TASKS
    ...
    def has_control_receipt(self, request_id):
        ...
        return isinstance(request_id, str) and request_id in self.journal.get(
            "control_receipts", {}
        )
    ...
    def plan_control(self, action, session_id, request_id, based_on_revision, now=None):
        ...
        if receipt is not None:
            if not isinstance(receipt, dict) or receipt.get("payload") != payload:
                raise TaskConflict(
                    "That record request id was already used for a different change."
                )
            return deepcopy(self.journal)
        if based_on_revision != revision:
            raise TaskConflict(
                "Your activity records changed while this request was being prepared. Review them and try again."
            )
        now = self._now(now)
        ...
        if action == "unlink" and (
            "trashed_at" in session
            or not session.get("related_task_ids")
        ...
        updated["sessions"] = [
            item for item in updated.get("sessions", []) if isinstance(item, dict)
        ]
        ...
        if action == "trash":
            ...
            updated["active"] = (
                None if updated.get("active") == session_id else updated.get("active")
            )
        elif action == "restore":
        ...
        cutoff = now - RETENTION_SECONDS
        updated["sessions"] = [
            item for item in updated["sessions"] if item.get("last_seen", 0) >= cutoff
        ][-MAX_SESSIONS:]
        active_id = updated.get("active")
        if not any(
            item.get("id") == active_id and item.get("end") is None and "trashed_at" not in item
        ...
        updated_receipts[request_id] = {
            "payload": payload,
            "revision": updated["manual_revision"],
            "at": now,
        }
