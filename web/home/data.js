...
export function homeData(snapshot) {
  const supported =
    ...
    Array.isArray(snapshot.tasks?.tasks) &&
    snapshot.tasks.tasks.every(
      (task) =>
  ...
  return {
    ...
    focus:
      tasks.find((task) => task.status === 'open' && task.id === snapshot.tasks.focus_id) || null,
...
export function conversationEntries(snapshot, localPending) {
  ...
  if (
    localPending &&
    pending?.request_id !== localPending.request_id &&
    !snapshot?.accepted_request_ids?.includes(localPending.request_id)
  ) {
    messages.push({
      id: localPending.request_id,
      role: 'user',
      text: localPending.text,
      delivery: localPending.status,
    });
  }
  return messages;
}
...
export function notificationEntry(snapshot, target) {
  ...
  return (
    (Array.isArray(snapshot?.messages) ? snapshot.messages : []).find(
      (entry) =>
        entry?.id === target.messageId &&
        entry.event_id === target.eventId &&
        entry.role === 'assistant' &&
        entry.origin === 'check_in',
    ) || null
  );
}
