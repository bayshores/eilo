...
const byId = (state, mode = 'wide') =>
  new Map(projectLayout(state, mode).map((item) => [item.id, item]));
const visibleIds = (state, mode = 'wide', rows = 4) =>
  projectLayout(state, mode)
    .filter((item) => item.y + item.h <= rows)
    .map((item) => item.id)
    .sort();
...
function assertVisibleHomeIsUsable(state, expectedIds, mode = 'wide', rows = 4) {
  ...
  for (let index = 0; index < visible.length; index++) {
    for (let peer = index + 1; peer < visible.length; peer++) {
      assert.equal(
        rectsOverlap(visible[index], visible[peer]),
        false,
        `${visible[index].id} and ${visible[peer].id} do not overlap`,
      );
    }
  }
}
...
test('resizeWithinHome grows Today elastically while retaining the docked default Home', () => {
  ...
  assert.deepEqual(
    [
      layout.get('today-1').x,
      layout.get('today-1').y,
      layout.get('today-1').w,
      layout.get('today-1').h,
    ],
    [0, 0, 7, 4],
  );
  assert.deepEqual(
    [
      layout.get('goals-1').x,
      layout.get('goals-1').y,
      layout.get('goals-1').w,
      layout.get('goals-1').h,
    ],
    [7, 0, 5, 2],
  );
  assert.deepEqual(
    [
      layout.get('progress-1').x,
      layout.get('progress-1').y,
      layout.get('progress-1').w,
      layout.get('progress-1').h,
    ],
    [7, 2, 5, 2],
  );
...
test('resizeWithinHome caps growth at readable peer widths and reverses shared-edge shrinkage', () => {
  ...
  assert.deepEqual(
    [capped.get('goals-1').x, capped.get('goals-1').w],
    [8, CATALOG.goals.sizes.small.w],
  );
  assert.deepEqual(
    [capped.get('progress-1').x, capped.get('progress-1').w],
    [8, CATALOG.progress.sizes.medium.w],
  );
...
test('resizeWithinHome preserves omitted height, anchors the target, and leaves responsive modes alone', () => {
  ...
  assert.deepEqual(next.widgets.find((widget) => widget.id === 'today-1').footprints.compact, {
    w: 3,
    h: 2,
  });
  assert.deepEqual(next.widgets.find((widget) => widget.id === 'today-1').footprints.stacked, {
    w: 1,
    h: 5,
  });
...
test('resizeWithinHome grows and shrinks a vertical shared edge while retaining every visible peer', () => {
  ...
  assert.deepEqual(
    [
      grownLayout.get('top').x,
      grownLayout.get('top').y,
      grownLayout.get('top').w,
      grownLayout.get('top').h,
    ],
    [0, 0, 6, 3],
  );
  assert.deepEqual(
    [
      grownLayout.get('bottom').x,
      grownLayout.get('bottom').y,
      grownLayout.get('bottom').w,
      grownLayout.get('bottom').h,
    ],
    [0, 3, 6, 2],
  );
...
test('resizeWithinHome supports compact and stacked constraints without moving absent modes', () => {
  const compact = resizeWithinHome(dockedDefault(), {
    id: 'today-1',
    w: 6,
    h: 2,
    mode: 'compact',
  ...
  const stacked = resizeWithinHome(dockedDefault(), {
    id: 'today-1',
    w: 99,
    h: 3,
    mode: 'stacked',
...
test('resizeWithinHome keeps an offset target anchored and respects the far Home edge', () => {
  ...
  const layout = byId(next);
  assert.deepEqual(
    [layout.get('today').x, layout.get('today').y, layout.get('today').w, layout.get('today').h],
    [3, 0, 5, 4],
  );
...
test('resizeWithinHome returns its exact input for invalid, unchanged, or infeasible visible requests', () => {
  ...
  const dense = normalizeState({
    version: 1,
    widgets: [
      { id: 'a', type: 'progress', size: 'small' },
      { id: 'b', type: 'progress', size: 'small' },
      { id: 'c', type: 'progress', size: 'small' },
      { id: 'd', type: 'progress', size: 'small' },
    ],
    positions: {
      wide: [
        { id: 'a', x: 0, y: 0 },
...
test('fitWithinHome recovers the old resizeTo overflow without resetting Today width', () => {
  ...
  const broken = updateLayout(before, {
    type: 'resizeTo',
    id: 'today-1',
    w: 7,
    h: 4,
  ...
  assert.ok(
    visibleIds(broken).length < 3,
    'the legacy generic settle puts a default peer into More',
  );
...
test('fitWithinHome is pure, does not touch other modes or hidden widgets, and no-ops by identity', () => {
  ...
  assert.deepEqual(
    fixed.widgets.find((widget) => widget.id === 'clock-hidden').footprints.compact,
    { w: 3, h: 2 },
  );
...
test('resizeWithinHome preserves Home invariants across deterministic mixed widget layouts', () => {
  ...
  for (let caseIndex = 0; caseIndex < 100; caseIndex++) {
    ...
    assert.deepEqual(
      [afterTarget.x, afterTarget.y],
      [target.x, target.y],
      `case ${caseIndex} retains target anchor`,
    );
    for (const item of layout) {
      const minimum =
        mode === 'wide' ? CATALOG[item.type].sizes.small.w : mode === 'compact' ? 3 : 1;
      assert.ok(
        item.x >= 0 && item.y >= 0 && item.x + item.w <= MODES[mode] && item.y + item.h <= 192,
        `case ${caseIndex} keeps ${item.id} bounded`,
      );
      assert.ok(item.w >= minimum && item.h >= 2, `case ${caseIndex} keeps ${item.id} readable`);
    }
    for (let index = 0; index < layout.length; index++)
      for (let peer = index + 1; peer < layout.length; peer++) {
        assert.equal(
          rectsOverlap(layout[index], layout[peer]),
          false,
          `case ${caseIndex} has no overlaps`,
        );
      }
    for (const otherMode of modes.filter((candidate) => candidate !== mode)) {
      assert.deepEqual(
        next.positions[otherMode],
        before.positions[otherMode],
        `case ${caseIndex} does not write ${otherMode} metadata`,
      );
      assert.deepEqual(
        projectLayout(next, otherMode),
        projectLayout(before, otherMode),
        `case ${caseIndex} preserves ${otherMode} layout`,
      );
    }
  }
});
