...
const snapshot = (chatId, sessionId = null, overrides = {}) => ({
  schema_version: 2,
  revision: `state:${chatId}:${sessionId || 'new'}`,
  ...
  workspace: { revision: 4, active_chat_id: chatId, projects: [], chats: [] },
  ...overrides,
});
...
test('a first native session id keeps the catalog chat draft and sends its chat id', async () => {
  ...
  assert.deepEqual(JSON.parse(h.calls.at(-1).body), {
    text: 'Keep this through native creation',
    request_id: ID,
    chat_id: 'chat-first',
  });
...
test('a v1 outbox migrates into its matching first catalog chat and catalog controls use catalog revision', async () => {
  const storage = memory();
  storage.setItem(
    'eilo:home:conversation-draft:v1',
    JSON.stringify({ conversationId: 'native-old', draft: 'Old draft', pending: null }),
  );
  ...
  h.replies.push(
    response(
      snapshot('chat-old', 'native-old', {
        workspace: { revision: 7, active_chat_id: 'chat-old', projects: [], chats: [] },
      }),
    ),
  );
