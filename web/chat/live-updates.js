...
export function createLiveUpdates({
  ...
  const queue = createUpdateQueue({
    ...
    saveSeen: (id, ids) => {
      try {
        localStorage.setItem(prefix + id, JSON.stringify(ids));
      } catch {
        /* Cross-tab update acknowledgement is optional when browser storage is blocked. */
      }
    },
  });
  ...
  panel.querySelector('.live-update-open').addEventListener('click', () => {
    openConversation();
    dismiss();
  });
  ...
  function schedule() {
    ...
    if (
      ...
      timer = setTimeout(dismiss, Math.max(12000, Math.min(30000, active.text.length * 65)));
  }
  const visible = () =>
  ...
  document
    ...
    .forEach((dialog) => dialogs.observe(dialog, { attributes: true, attributeFilter: ['open'] }));
  window.addEventListener('storage', (event) => {
    if (event.key?.startsWith(prefix) && latest) update(latest);
  });
  return { update };
}
