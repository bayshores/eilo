...
test('opening conversation preserves the next update and pauses dismissal while covered', () => {
  const original = Object.fromEntries(
    ['document', 'window', 'localStorage', 'MutationObserver', 'setTimeout', 'clearTimeout'].map(
  ...
  document.querySelector = (selector) =>
    selector === '.app-window' ? app : selector === 'dialog[open]' && dialog.open ? dialog : null;
  ...
  try {
    ...
    const state = {
      snapshot: { conversation_id: 'test-conversation', messages: [], accountability: {} },
    };
    updates.update(state);
    state.snapshot.messages = [1, 2].map((number) => ({
      id: `native-${number}`,
      event_id: `check-${number}`,
      role: 'assistant',
      origin: 'check_in',
      text: `Update ${number}`,
    }));
