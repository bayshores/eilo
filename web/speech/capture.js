export function encodeWav(chunks, sampleRate) {
  ...
  if (!length || length > sampleRate * 120)
    throw new Error('Record a phrase of up to two minutes.');
  ...
  for (let i = 0; i < count; i++) {
    const begin = Math.floor((i * sampleRate) / 16000),
      end = Math.max(begin + 1, Math.floor(((i + 1) * sampleRate) / 16000));
    let sum = 0,
      n = 0;
    for (let j = begin; j < Math.min(end, length); j++) {
      sum += Number.isFinite(input[j]) ? input[j] : 0;
      n++;
    }
...
export function createSpeechCapture({
  ...
  WorkletNodeClass = globalThis.AudioWorkletNode,
  ...
  async function start() {
    ...
    let acquired = null,
      localContext = null;
    try {
      if (!mediaDevices?.getUserMedia || !AudioContextClass || !WorkletNodeClass)
        throw new Error('This browser cannot capture speech. You can still type your message.');
      localContext = new AudioContextClass({ sampleRate: 16000 });
      ...
      source = localContext.createMediaStreamSource(acquired);
      processor = new WorkletNodeClass(localContext, 'eilo-microphone');
      gain = localContext.createGain();
      gain.gain.value = 0;
      processor.port.onmessage = (event) => {
        ...
        if (
          ...
          return;
        const room = localContext.sampleRate * 120 - frames,
          part = event.data.samples.subarray(0, Math.max(0, room));
        if (part.length) {
          chunks.push(part);
          frames += part.length;
          onLevel(Number.isFinite(event.data.level) ? event.data.level : 0);
        }
        if (frames >= localContext.sampleRate * 120 && state === 'recording') stop();
      };
      source.connect(processor);
      processor.connect(gain);
      gain.connect(localContext.destination);
      acquired.getTracks().forEach((track) =>
        track.addEventListener(
          'ended',
          () => {
            if (state === 'recording') stop({ cancel: true });
          },
          { once: true },
        ),
      );
      ...
      notify(
        ...
        error.name === 'NotAllowedError'
          ? 'Microphone access was not granted. Allow it in your browser when you want to use speech.'
  ...
  async function stop({ cancel = false } = {}) {
    ...
    if (state === 'transcribing') {
      ...
      abort?.abort();
      if (request)
        fetcher('/api/transcribe/cancel', {
          method: 'POST',
          headers: { 'X-Eilo-Client': 'local-chat', 'Content-Type': 'application/json' },
    ...
    try {
      ...
      const response = await fetcher('/api/transcribe', {
        ...
        signal: abort.signal,
        headers: { 'X-Eilo-Client': 'local-chat', 'Content-Type': 'application/json' },
        body: JSON.stringify({ request_id: request, audio: btoa(binary) }),
      });
      const result = await response.json();
      if (current !== generation) return;
      if (!response.ok || typeof result.text !== 'string')
        throw new Error(result.error || 'Local transcription could not finish.');
      request = null;
      onTranscript(result.text);
      notify('idle', 'Transcript added. Review it before sending.');
    } catch (error) {
      if (current === generation)
        notify('error', error.message || 'Transcription could not finish.');
    } finally {
      if (current === generation) {
        request = null;
        abort = null;
