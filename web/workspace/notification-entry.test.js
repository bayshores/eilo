...
test('notification opens only its existing assistant check-in in the current conversation', () => {
  ...
  for (const change of [{ conversationId: 'two' }, { messageId: '13' }, { eventId: 'other' }])
    assert.equal(notificationEntry(snapshot, { ...target, ...change }), null);
  assert.equal(
    notificationEntry({ ...snapshot, messages: [{ ...message, origin: 'user' }] }, target),
    null,
  );
