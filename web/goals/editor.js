...
function quantity(value, label, { optional = false, minimum = 0 } = {}) {
  ...
  if (!/^\d+$/.test(text) || Number(text) < minimum || Number(text) > 10000)
    throw new Error(`${label} must be a whole number from ${minimum} to 10,000.`);
  return Number(text);
}
...
export function taskEditOperations(task, values, tempId = 'new_manual') {
  ...
  const changes = Object.fromEntries(
    Object.entries(fields).filter(([key, value]) => value !== (task[key] ?? null)),
  );
  const count =
    target !== null && task.status === 'open'
      ? quantity(values.completed_count, 'Completed')
      : task.completed_count;
  if (target !== null && count > target)
    throw new Error('Completed progress cannot exceed the target.');
  const edit = Object.keys(changes).length ? { op: 'edit', task_id: task.id, ...changes } : null;
  const progress =
    target !== null && task.status === 'open' && count !== task.completed_count
      ? { op: 'progress', task_id: task.id, completed_count: count }
      : null;
  // Lower progress before a smaller target; expand a target before increasing progress.
  return target !== null && target < task.completed_count
    ? [progress, edit].filter(Boolean)
    : [edit, progress].filter(Boolean);
}
...
export function goalActionOperations(task, action, focusId = null) {
  ...
  if (!['complete', 'cancel', 'reopen', 'delete', 'restore'].includes(action))
    throw new Error('Choose a supported goal action.');
...
export function undoGoalOperations(task, action, focusId = null) {
  ...
  if (action === 'complete' || action === 'cancel') {
    if (task.status === 'open')
      return [
        { op: 'reopen', task_id: task.id },
        ...(task.target_count !== null
          ? [{ op: 'progress', task_id: task.id, completed_count: task.completed_count }]
          : []),
        ...restoreFocus,
      ];
