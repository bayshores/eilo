...
export function isBriefingSourcesSnapshot(value) {
  return (
    ...
    value.accounts.every(
      (account) => account && typeof account.id === 'string' && ACCOUNT_STATES.has(account.state),
    )
  );
}
...
export function formatCheckedAt(value) {
  ...
  return Number.isNaN(date.getTime())
    ...
    : `Last checked ${date.toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}`;
}
...
export function mountBriefingSources(
  container,
  {
    fetcher = fetch,
  ...
  if (!(container instanceof Element))
    throw new TypeError('A Briefing Sources container is required.');
  ...
  const scheduleAuthorizationPoll = () => {
    clearTimer();
    if (!destroyed && snapshot?.authorizing)
      timer = setTimeout(() => refresh({ quiet: true }), AUTH_POLL_MS);
  };
  const focusKey = () => {
  ...
  const transport = async (method, payload) => {
    ...
    try {
      ...
      if (!response.ok)
        throw Object.assign(new Error(data?.error || 'Briefing sources request failed.'), {
  ...
  const accept = (next, preserve = '') => {
    if (!isBriefingSourcesSnapshot(next))
      throw new Error('Briefing sources returned an unexpected response. Try again.');
  ...
  const refresh = async ({ quiet = false } = {}) => {
    ...
    try {
      accept(await transport('GET'), prior);
    } catch (error) {
      if (!destroyed) {
        setStatus(messageFor(error), true);
        if (!quiet) render(prior);
      }
    }
  };
  const mutate = async (action, extra = {}, focusAfter = '') => {
    ...
    render(prior);
    try {
      const next = await transport('POST', {
        action,
      ...
      if (!destroyed) {
        setStatus(messageFor(error), true);
        render(focusAfter || prior);
        if (error?.status === 409) refresh({ quiet: true });
      }
      return null;
    } finally {
      busy = false;
      if (!destroyed) render(focusAfter || prior);
    }
  };
  const connect = async () => {
    if (!consent) return;
  ...
  const beginAuthorization = async (focusAfter) => {
    ...
    try {
      if (!(await openAuthorization(next.authorization_url))) {
        setStatus('Google sign-in did not open. Try Connect Gmail again.', true);
        render('briefing-sources-status');
      }
    } catch {
      setStatus('Google sign-in did not open. Try Connect Gmail again.', true);
      render('briefing-sources-status');
  ...
  const render = (restore = '') => {
    ...
    title.id = 'briefing-sources-title';
    title.tabIndex = -1;
    title.dataset.focus = 'briefing-sources-title';
    ...
    if (!snapshot) {
      ...
      if (typeof snapshot.error === 'string' && snapshot.error)
        content.append(el('p', 'briefing-sources__error-copy', snapshot.error));
      const inbox = el('section', 'briefing-sources__group');
      inbox.setAttribute('aria-labelledby', 'briefing-inbox-title');
      const inboxTitle = el('h3', section === 'gmail' ? 'sr-only' : '', 'Gmail');
      ...
      if (snapshot.authorizing) {
        ...
        if (authorizationUrl)
          actions.append(
            button(
              'Open Google sign-in',
              async () => {
                if (!(await openAuthorization(authorizationUrl)))
                  setStatus('Google sign-in did not open. Try again.', true);
              },
              'briefing-sources__button briefing-sources__button--primary',
            ),
          );
        actions.append(
          button('Cancel connection', () => mutate('cancel_gmail', {}, 'briefing-inbox-title')),
        );
        ...
        check.dataset.focus = 'gmail-consent';
        check.addEventListener('change', () => {
          consent = check.checked;
          render('gmail-consent');
        });
        consentLabel.append(
          check,
          el(
        ...
        const actions = el('div', 'briefing-sources__actions');
        const start = button(
          reconnecting ? 'Reconnect Gmail' : 'Connect Gmail',
          connect,
          'briefing-sources__button briefing-sources__button--primary',
        );
        start.disabled = busy || !consent;
        start.dataset.focus = 'connect-gmail';
        ...
        snapshot.accounts.forEach((account) => {
          ...
          const copy = el('div', 'briefing-sources__account-copy');
          copy.append(el('strong', '', account.email));
          const state =
            account.state === 'reauth_required'
              ...
              : account.state === 'paused'
                ? 'Paused'
                : account.enabled
                  ? 'Used in answers and briefings'
                  : 'Not used in answers and briefings';
          copy.append(el('span', '', state));
          if (account.last_checked_at)
            copy.append(el('small', '', formatCheckedAt(account.last_checked_at)));
          if (account.error)
            copy.append(el('small', 'briefing-sources__error-copy', account.error));
          const toggle = el('label', 'briefing-sources__switch');
          const input = document.createElement('input');
          input.type = 'checkbox';
          input.setAttribute('role', 'switch');
          input.checked = Boolean(account.enabled);
          input.disabled = busy;
          input.dataset.focus = `mail-${account.id}`;
          input.setAttribute('aria-label', `Use ${account.email} in answers and briefings`);
          input.addEventListener('change', () =>
            mutate(
              'set_mail_enabled',
              { account_id: account.id, enabled: input.checked },
              input.dataset.focus,
            ),
          );
          ...
          if (account.state === 'reauth_required')
            actions.append(
              button('Reconnect', () => {
                connectMode = account.id;
                consent = false;
                render(`mail-${account.id}`);
              }),
            );
          if (removeId === account.id) {
            const confirm = el('div', 'briefing-sources__confirm');
            confirm.append(
              el(
                'p',
                '',
            ...
            const yes = button(
              ...
              () => {
                removeId = '';
                mutate('remove_gmail', { account_id: account.id }, 'briefing-inbox-title');
              },
              'briefing-sources__button briefing-sources__button--danger',
            );
            yes.dataset.focus = `remove-${account.id}`;
            confirm.append(
              yes,
              button('Keep inbox', () => {
                removeId = '';
            ...
            const remove = button(
              ...
              () => {
                removeId = account.id;
                render(`remove-${account.id}`);
              },
              'briefing-sources__button briefing-sources__button--quiet',
            );
            remove.dataset.focus = `remove-${account.id}`;
      ...
      const calendarGroup = el('section', 'briefing-sources__group');
      calendarGroup.setAttribute('aria-labelledby', 'briefing-calendar-title');
      const calendarTitle = el('h3', '', 'Selected calendars');
      calendarTitle.id = 'briefing-calendar-title';
      calendarGroup.append(calendarTitle);
      const calendarLabel = el('label', 'briefing-sources__toggle-row');
      const calendarInput = document.createElement('input');
      calendarInput.type = 'checkbox';
      calendarInput.checked = Boolean(calendar.enabled);
      calendarInput.disabled = busy || !calendar.available || !calendar.selected_count;
      calendarInput.dataset.focus = 'calendar-enabled';
      ...
      calendarGroup.append(calendarLabel);
      if (section !== 'gmail') content.append(calendarGroup);
    }
