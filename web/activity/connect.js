...
function render(view) {
  ...
  const unavailable = !capable
    ? 'Open this page in Chrome to connect an approved extension.'
    : !helperReady
      ? 'The local activity helper is unavailable. Check eïlo Home or try again shortly.'
      : null;
  elements.pill.textContent =
    state === 'active' ? 'Sharing' : state === 'paused' ? 'Paused' : 'Idle';
  elements.pill.dataset.state =
    state === 'active' ? 'active' : state === 'paused' ? 'paused' : unavailable ? 'error' : 'idle';
  elements.status.textContent =
    unavailable ||
    (state === 'active'
      ? 'Chrome activity is connected.'
      : state === 'paused'
        ? 'Sharing is paused.'
        : online
          ? 'Ready when you are.'
  ...
  elements.detail.textContent =
    state === 'active'
      ? 'eïlo can request a fresh approved-tab signal only while this tab remains open.'
      : state === 'paused'
        ? 'Nothing is currently requested from Chrome. You can reconnect when you choose.'
