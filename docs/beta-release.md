...
Still required before any future release: clean-machine Electron/Chrome onboarding; real capture/privacy checks with consent; verified signed helper/runtime behavior; version update/rollback; full performance soak; user testing; and an explicit decision to distribute. The visual model path remains unavailable until its fixture succeeds. See [the architecture](architecture.md).
