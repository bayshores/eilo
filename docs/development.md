...
`start` opens Electron from the root, prepares missing native helpers, and lets Electron own the local service on **http://127.0.0.1:8765/home/**. Stop the app with **felis → Quit felis**. Do not separately start a second service for the desktop app. The source checkout is required for this development build. Its felis menu includes **Preview check-in overlay** for a local rendering check; it does not inspect activity, create a conversation message, or send an OS notification.
...
| Port 8765 is already in use                      | Use the already-running felis app, or quit it normally before starting another copy.                                                        |
