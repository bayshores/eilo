...
- **Keep a Windows route.** The desktop shell can be shared, but the current eïlo backend includes macOS/POSIX-specific helper, locking and process code. Windows packaging and behavior need their own adaptation and verification; an Electron choice does not make the current backend Windows-ready. [Current server](../app/server.py#L12), [foreground helper](../activity/native/EiloActivityHelper.swift#L20).
