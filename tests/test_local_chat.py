...
class PendingRequestTests(unittest.IsolatedAsyncioTestCase):
        ...
        async def command(arguments, **kwargs):
            ...
            if arguments[0] == "--input":
                ...
                for row in self.records[0]["messages"]:
                    row.setdefault("display_metadata", {})["request_id"] = self.request[
                        "request_id"
                    ]
                    ...
                    json.dumps(
                        {
                            ...
                            "user_id": self.records[0]["messages"][-2]["id"],
                            "proposal": {
                                "request_id": self.request["request_id"],
                                "based_on_revision": 0,
                                "kind": "chat",
            ...
            if arguments[0] == "--finalize":
                ...
                assistant["display_metadata"].update(
                    {
                        "published": True,
                        "public_reply": publication["public_reply"],
    ...
    async def finish(self, messages):
        ...
        user["display_metadata"] = {
            "lane": "eilo_human",
            "request_id": self.request["request_id"] if self.request else "pending",
        }
    ...
    async def test_accepted_message_visible_before_native_work_and_after_reload(self):
        ...
        self.assertEqual(
            ...
            {
                "request_id": "request_pending_001",
                "text": "Benign pending test",
                "status": "accepted",
            },
        )
        ...
        self.assertEqual(
            (await self.chat.send("Benign pending test", "request_pending_001"))["pending_message"],
            state["pending_message"],
        )
        await self.finish(
            [
                {"id": 1, "role": "user", "content": "Benign pending test"},
    ...
    async def test_repeated_text_reconciles_only_after_the_native_anchor(self):
        self.chat.messages = [
            {"id": "1", "role": "user", "text": "Repeated text"},
        ...
        await self.finish(
            [
                {"id": 1, "role": "user", "content": "Repeated text"},
    ...
    async def test_failure_before_acceptance_does_not_create_pending_or_native_call(self):
        ...
        with patch(
            "app.chat_service.check_config",
            side_effect=ChatError("Fixture configuration rejection"),
        ):
                ...
                [
                    {
                        ...
                        "display_metadata": {
                            "lane": "eilo_human",
                            "request_id": "request_first_turn_001",
                        },
                    },
                ]
            )
    ...
    async def test_long_poll_wakes_for_acceptance_and_native_completion(self):
        ...
        await self.finish(
            [
                {"id": 1, "role": "user", "content": "Wake the display"},
                {"id": 2, "role": "assistant", "content": "Received."},
            ]
        )
        update = await asyncio.wait_for(waiting, 1)
