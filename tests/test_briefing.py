...
from app.briefing import MAX_CALLS, MAX_CONTEXT, MAX_THREADS, Briefing, SourceTurn
...
class FakeMail:
    ...
    def snapshot(self):
        return {
            "accounts": [
                {key: value for key, value in account.items() if key != "sub"}
                for account in self.accounts
            ],
            "calendar": {
                "available": True,
                "enabled": self.calendar_ok,
                "selected_count": 1,
...
class BriefingTests(unittest.TestCase):
    ...
    def test_undiscovered_thread_id_is_blocked_without_provider_read(self):
        result = self.run_async(
            self.turn.read(
                "eilo_read_mail", {"account_id": self.account, "thread_id": "not-discovered"}
            )
        )
    ...
    def test_search_window_and_read_budgets_are_bounded(self):
        ...
        with mock.patch("app.briefing.search_threads", fake_search):
            result = self.run_async(
                self.turn.read(
                    "eilo_search_mail",
                    {"account_id": self.account, "days": 90, "query": "deadline"},
                )
            )
        ...
        with self.assertRaises(CalendarError):
            self.run_async(
                self.turn.read("eilo_search_mail", {"account_id": self.account, "days": 91})
            )
        ...
        with mock.patch("app.briefing.read_thread", fake_thread):
            for index in range(MAX_THREADS):
                self.assertEqual(
                    self.run_async(
                        self.turn.read(
                            "eilo_read_mail", {"account_id": self.account, "thread_id": f"t{index}"}
                        )
                    )["receipt"]["status"],
                    "ok",
                )
            limited = self.run_async(
                self.turn.read(
                    "eilo_read_mail", {"account_id": self.account, "thread_id": f"t{MAX_THREADS}"}
                )
            )
    ...
    def test_call_and_context_budgets_and_whole_thread_excerpt_limit(self):
        ...
        with mock.patch("app.briefing.read_thread", long_thread):
            read = self.run_async(
                per_thread.read(
                    "eilo_read_mail", {"account_id": self.account, "thread_id": "thread"}
                )
            )
    ...
    def test_permission_change_invalidates_turn_and_prevents_returning_fetched_data(self):
        ...
        with mock.patch("app.briefing.search_threads", revoked_search):
            result = self.run_async(
                self.turn.read("eilo_search_mail", {"account_id": self.account})
            )
        self.assertEqual(result["receipt"]["status"], "unavailable")
        self.assertIsNone(result["data"])
    ...
    def test_workflow_reports_real_failure_and_never_persists_raw_mail(self):
        ...
        async def successful_thread(request, thread_id, *, max_messages, max_chars):
            return {
                "id": thread_id,
                "messages": [
                    {"id": "m", "thread_id": thread_id, "excerpt": raw, "truncated": False}
                ],
                "truncated": False,
            }
        ...
        with mock.patch("app.briefing.read_thread", successful_thread):
            result = self.run_async(
                self.turn.read("eilo_read_mail", {"account_id": self.account, "thread_id": "t"})
            )
        self.assertEqual(result["receipt"]["status"], "ok")
        ...
        with mock.patch("app.briefing.search_threads", unavailable):
            failed_result = self.run_async(
                failed.read("eilo_search_mail", {"account_id": self.account})
            )
