...
from app.chat_catalog import CatalogError, import_sessions, snapshot_catalog
...
class WorkspaceCatalogControllerTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.config = patch("app.chat_service.check_config")
        self.config.start()
        ...
        self.chat.meta.update(
            ...
            accepted_requests=["request-old"],
            source_settings={"gmail": True},
            activity_marker={"state": "paused"},
        )
        self.chat.changed()
    ...
    def body(self, action, **values):
        return {
            "action": action,
            "based_on_revision": snapshot_catalog(self.chat.meta)["revision"],
            **values,
        }
    ...
    async def test_failed_destination_read_does_not_change_current_pointer_or_visible_messages(
        self,
    ):
        first, second = await self.make_second_chat()
    ...
    async def test_multiple_chats_require_explicit_active_chat_id_for_send(self):
        ...
        with self.assertRaises(CatalogError) as wrong:
            await self.chat.send(
                "No network call", "request-workspace-002", chat_id="chat-not-active"
            )
...
class SessionImportTests(unittest.TestCase):
    ...
    def test_import_rows_never_replaces_current_or_copies_messages(self):
        ...
        self.assertEqual(
            snapshot_catalog(imported)["active_chat_id"], snapshot_catalog(meta)["active_chat_id"]
        )
        older = [
            row
            for row in imported["chat_catalog"]["chats"]
            if row["conversation"]["session_id"] == "old"
        ][0]
