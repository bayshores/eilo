...
class TaskTrashTests(unittest.TestCase):
    def apply(self, state, operations, *, request, revision=None, now=100):
        return apply_operations(
            ...
            request_id=request,
            source={"kind": "human", "session_id": "fixture"},
            now=now,
    ...
    def test_delete_is_atomic_and_clears_focus_without_losing_task_data(self):
        state = self.seeded()
        state, _ = self.apply(
            state, [{"op": "focus", "task_id": "task_one"}], request="request_focus"
        )
        deleted, _ = self.apply(
            state, [{"op": "delete", "task_id": "task_one"}], request="request_delete", now=123
        )
        ...
        restored, _ = self.apply(
            deleted, [{"op": "restore", "task_id": "task_one"}], request="request_restore"
        )
        ...
        with self.assertRaises(TaskError):
            self.apply(
                state,
                [
                    {"op": "delete", "task_id": "task_one"},
                    {"op": "edit", "task_id": "task_one", "title": "Nope"},
                ],
                request="request_atomic",
            )
        ...
        for prior, expected_count in (("open", 0), ("completed", 3), ("cancelled", 0)):
            with self.subTest(prior=prior):
                ...
                if prior == "completed":
                    ...
                    state, _ = self.apply(
                        state, [{"op": "cancel", "task_id": "task_one"}], request="request_cancel"
                    )
                deleted, _ = self.apply(
                    state,
                    [{"op": "delete", "task_id": "task_one"}],
                ...
                repeated, receipt = self.apply(
                    deleted,
                    [{"op": "delete", "task_id": "task_one"}],
                    request="request_delete_again",
                    now=456,
                )
                ...
                restored, _ = self.apply(
                    repeated, [{"op": "restore", "task_id": "task_one"}], request="request_restore"
                )
                task = restored["tasks"][0]
                self.assertEqual(
                    (task["status"], task["completed_count"], restored["focus_id"]),
                    (prior, expected_count, None),
                )
    ...
    def test_deleted_task_rejects_every_non_trash_action_and_restore_requires_trash(self):
        state = self.seeded()
        deleted, _ = self.apply(
            state, [{"op": "delete", "task_id": "task_one"}], request="request_delete"
        )
        invalid = [
            {"op": "edit", "task_id": "task_one", "title": "Nope"},
        ...
        with self.assertRaises(TaskError):
            self.apply(
                state, [{"op": "restore", "task_id": "task_one"}], request="request_not_deleted"
            )
    ...
    def test_public_state_excludes_tombstones_and_same_request_and_stale_checks_stay_safe(self):
        ...
        duplicate, receipt = self.apply(
            deleted, [{"op": "restore", "task_id": "task_one"}], request="request_delete"
        )
    ...
    def test_human_and_event_source_contracts_describe_deleted_tasks_without_runtime_work(self):
        ...
        self.assertIn(
            "Never restore a task unless the user explicitly\nasks", human_driver.SYSTEM_POLICY
        )
        self.assertIn(
            "Deleted tasks are never active work, eligible focus", event_driver.SYSTEM_POLICY
        )
        state = self.seeded()
        deleted, _ = self.apply(
            state, [{"op": "delete", "task_id": "task_one"}], request="request_delete"
        ...
        event = {
            ...
            "task_state": public_state(deleted),
        }
        with self.assertRaises(event_driver.InputError):
            event_driver.validate_input(event)
        live, _ = self.apply(
            deleted, [add("new_two", "Other", target=None, unit=None)], request="request_add"
        )
        admitted = event_driver.validate_input({**event, "task_state": public_state(live)})
        self.assertEqual(
            [task["status"] for task in admitted["task_state"]["tasks"]], ["deleted", "open"]
        )
