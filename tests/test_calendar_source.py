...
class CalendarSourceTests(unittest.TestCase):
    ...
    def test_calendar_pagination_and_reader_filter(self):
        ...
        self.assertEqual(
            self.run_async(fetch_calendars(request)),
            [
    ...
    def test_calendar_field_mask_known_empty_response_and_duplicates(self):
        ...
        duplicate = FakeRequest(
            [
                {
                    "items": [{"id": "one", "summary": "One", "accessRole": "reader"}],
                    "nextPageToken": "two",
                },
    ...
    def test_known_selection_and_safe_calendar_url(self):
        ...
        self.run_async(
            fetch_events(
                request, calendars, ["team/a@example.com"], datetime(2026, 9, 9, 16, tzinfo=UTC)
            )
        )
        ...
        with self.assertRaisesRegex(CalendarSourceError, "at most 10"):
            self.run_async(
                fetch_events(
                    FakeRequest([]),
                    [{"id": str(i), "name": str(i)} for i in range(11)],
    ...
    def test_event_filters_all_day_and_stable_recurring_instance_ids(self):
            ...
            [
                {
                    "items": [
                        ...
                        },
                        {
                            "id": "instance-b",
                            "summary": "Standup",
                            "start": {"dateTime": "2026-09-11T09:00:00-07:00"},
                            "end": {"dateTime": "2026-09-11T09:30:00-07:00"},
                        },
                    ]
                }
            ]
        ...
        events = self.run_async(
            fetch_events(request, calendars, ["cal"], datetime(2026, 9, 9, 9, tzinfo=UTC))
        )
        self.assertEqual(
            [(event["start"], event["end"], event["all_day"]) for event in events[:1]],
            [("2026-09-10", "2026-09-13", True)],
        )
        ...
        self.assertEqual(
            events[1]["id"],
            self.run_async(
                fetch_events(FakeRequest([{"items": [repeated_instance]}]), calendars, ["cal"])
            )[0]["id"],
        )
    ...
    def test_utc_window_and_event_pagination_errors(self):
        ...
        with self.assertRaisesRegex(CalendarSourceError, "repeated"):
            self.run_async(
                fetch_events(request, calendars, ["cal"], datetime(2026, 9, 9, 23, tzinfo=UTC))
            )
        ...
        with self.assertRaisesRegex(CalendarSourceError, "invalid .*page token"):
            self.run_async(
                fetch_events(FakeRequest([{"items": [], "nextPageToken": 7}]), calendars, ["cal"])
            )
    ...
    def test_event_known_empty_response_and_cross_page_deduplication(self):
        ...
        self.assertEqual(
            self.run_async(
                fetch_events(FakeRequest([{"kind": "calendar#events"}]), calendars, ["cal"])
            ),
            [],
        )
        event = {
