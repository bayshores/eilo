...
class HomeRouteTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_home_redirect_and_live_canonical_marker(self):
        ...
        self.assertIn('<html lang="en" data-typography="plex" data-source="live">', await page.text())
    ...
    async def test_extension_setup_returns_to_current_activity_and_uses_connection_page(self):
        ...
        parser = Links(); parser.feed(await setup.text())
    ...
    async def test_desktop_identity_uses_existing_local_boundaries(self):
        ...
        self.assertEqual(await response.json(), {"app": "eilo", "protocol": 1, "workspace": str(ROOT)})
    ...
    async def test_only_allowlisted_home_assets_are_served(self):
        for path in ("/home/index.html", "/home/layout.js", "/home/workspace-views.js", "/home/workspace-views.css", "/home/item-controls.js", "/home/item-controls.css", "/home/task-edit.js", "/home/goals-data.js", "/home/calendar-connection.js", "/home/calendar-connection.css", "/home/calendar-agenda.js", "/home/assets/fonts/ibm-plex-sans.woff2"):
            response = await self.client.get(path)
            self.assertEqual(response.status, 200, path)
        for path in ("/home/server.py", "/home/.state/local-chat.json", "/home/%2e%2e/.state/local-chat.json"):
            response = await self.client.get(path)
    ...
    async def test_checkin_status_assets_are_available_without_exposing_tests(self):
        for asset in ('checkin-center.js', 'checkin-data.js', 'checkin-center.css'):
            response = await self.client.get('/home/' + asset)
            self.assertEqual(response.status, 200, asset)
        response = await self.client.get('/home/checkin-data.test.js')
        self.assertEqual(response.status, 404)

    async def test_unconfigured_calendar_uses_real_connector_and_rejects_cross_site_access(self):
        response = await self.client.get('/api/integrations/google-calendar',headers={'X-Eilo-Client':'local-chat'})
        self.assertEqual(response.status,200)
        ...
        denied=await self.client.get('/api/integrations/google-calendar',headers={'X-Eilo-Client':'local-chat','Origin':'https://example.com'})
        self.assertEqual(denied.status,403)
        begin=await self.client.post('/api/integrations/google-calendar',headers={'X-Eilo-Client':'local-chat'},json={'action':'begin','based_on_revision':0})
