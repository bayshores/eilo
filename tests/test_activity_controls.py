...
from app.activity_ledger import ActivityLedger, MAX_CONTROL_RECEIPTS
...
class ActivityControlTests(unittest.TestCase):
    def ledger(self, state=None):
        ...
        return ActivityLedger({} if state is None else state, now=lambda: 1000,
    ...
    def test_trashing_active_closes_at_last_sample_and_next_sample_has_new_id(self):
        ...
        ledger = ActivityLedger({"activity_journal": updated}, now=lambda: 20, id_factory=lambda: "session-b")
        snap = ledger.snapshot(20)
        self.assertIsNone(snap["active_session"])
        self.assertEqual(snap["recent_sessions"], [])
        self.assertEqual((snap["trash_sessions"][0]["end"], snap["trash_sessions"][0]["observed_seconds"]), (15, 5))
    ...
    def test_restore_keeps_record_closed_and_totals_exclude_trash(self):
        ...
        self.assertEqual((session["end"], session["last_seen"], session["observed_seconds"]), (106, 106, 6))
        self.assertIsNone(restored_ledger.snapshot(110)["active_session"])
        self.assertEqual(restored_ledger.snapshot(110)["today_observed_seconds_by_origin"], {"https://leetcode.com": 6})

    def test_restore_does_not_stop_an_unrelated_active_session(self):
        ledger = self.ledger()
        ledger.observe(LEETCODE, 100)
        ledger.journal["sessions"].append({"id": "older", "origin": "https://leetcode.com", "start": 50,
                                           "end": 55, "last_seen": 55, "sample_count": 2,
                                           "observed_seconds": 5, "observed_by_utc_day": {}, "trashed_at": 60})
        updated = self.plan(ledger, "restore", "older", "restore-older", 0, 100)
        self.assertEqual(updated["active"], "session-a")
        self.assertIsNone(next(item for item in updated["sessions"] if item["id"] == "older").get("trashed_at"))
        self.assertIsNotNone(next(item for item in updated["sessions"] if item["id"] == "older")["end"])

    def test_expired_record_is_rejected_without_a_receipt_or_mutation(self):
        state = {"activity_journal": {"active": None, "sessions": [
            {"id": "expired", "origin": "https://leetcode.com", "start": 0, "end": 0,
             "last_seen": 0, "sample_count": 1, "observed_seconds": 0, "observed_by_utc_day": {}}]}}
    ...
    def test_unlink_dismisses_association_without_touching_tasks_or_consent(self):
        state = {"tasks": {"revision": 7, "tasks": [{"id": "task-1"}]}, "accountability": {"enabled": True}}
        ...
        self.assertEqual({"tasks": state["tasks"], "accountability": state["accountability"]}, before)
    ...
    def test_trash_stays_within_the_shared_retention_cap(self):
        sessions = [{"id": f"record-{index}", "origin": "https://leetcode.com", "start": 1000,
                     "end": 1000, "last_seen": 1000, "sample_count": 1, "observed_seconds": 0,
                     "observed_by_utc_day": {}} for index in range(128)]
    ...
    def test_receipts_are_bounded(self):
        state = {"activity_journal": {"active": None, "sessions": [], "manual_revision": 0,
                                      "control_receipts": {f"request-{index}": {"payload": {}}
                                                           for index in range(MAX_CONTROL_RECEIPTS)}}}
        ledger = self.ledger(state)
        ledger.observe(LEETCODE, 100)
