...
class TaskProjectionTests(unittest.TestCase):
    ...
    def test_multiple_adds_are_stable_and_can_reference_same_batch(self):
        state, _ = self.apply(
            initial_tasks(),
            [
                add("new_one", "Read notes"),
                add("new_two", "Solve problem"),
                {"op": "focus", "task_id": "new_two"},
            ],
            ids=("task_a", "task_b"),
        )
    ...
    def test_add_preserves_existing_tasks_and_focus(self):
        state, _ = self.apply(
            initial_tasks(),
            [add("new_one", "Existing"), {"op": "focus", "task_id": "new_one"}],
            ids=("task_a",),
        )
        updated, _ = self.apply(
            state, [add("new_two", "Another")], request="request_000002", ids=("task_b",)
        )
    ...
    def test_edit_progress_lifecycle_focus_and_break(self):
        state, _ = self.apply(
            initial_tasks(),
            [
                add("new_one", "Draft", target=5, unit="pages"),
        ...
        state, _ = self.apply(
            state,
            [
                {"op": "edit", "task_id": "task_a", "due_text": "next Friday"},
                {"op": "progress", "task_id": "task_a", "completed_count": 2},
                {"op": "break", "active": True},
            ],
            request="request_000002",
        )
        task = state["tasks"][0]
        self.assertEqual(
            (task["due_text"], task["completed_count"], state["break_active"]),
            ("next Friday", 2, True),
        )
        state, _ = self.apply(
            state,
            [{"op": "complete", "task_id": "task_a"}, {"op": "break", "active": False}],
            request="request_000003",
        )
        self.assertEqual(
            (state["tasks"][0]["status"], state["tasks"][0]["completed_count"], state["focus_id"]),
            ("completed", 5, None),
        )
        state, _ = self.apply(
            state,
            [
                {"op": "reopen", "task_id": "task_a"},
                {"op": "focus", "task_id": "task_a"},
                {"op": "cancel", "task_id": "task_a"},
            ],
    ...
    def test_stale_revision_and_duplicate_request_are_safe(self):
        state, _ = self.apply(
            initial_tasks(), [add("new_one", "Saved")], request="request_once", ids=("task_a",)
        )
        duplicate, receipt = self.apply(
            state, [add("new_two", "Ignored")], request="request_once", ids=("task_b",)
        )
    ...
    def test_legacy_migration_keeps_active_and_break_meaning(self):
        active = migrate_legacy(
            {
                "text": "Finish transfer form",
                "status": "active",
                "version": 3,
        ...
        self.assertEqual(
            (active["tasks"][0]["title"], active["focus_id"], active["break_active"]),
