...
class Chat:
    def __init__(self, directory, now):
        ...
        self.meta["tasks"], _ = apply_operations(self.meta["tasks"], [{"op": "add", "temp_id": "new_one", "title": "Practice", "due_text": None, "target_count": None, "unit": None}], based_on_revision=0, request_id="seed", source={"kind": "human"}, now=1, id_factory=lambda: "task-one")
    ...
    async def refresh(self): self.refresh_count += 1

    async def command(self, args, **kwargs):
        callback = kwargs.get("on_preview")
        if callback and self.emit_preview: callback("Writing")
        event_id = Path(args[args.index("--input") + 1]).stem.removeprefix("event-")
        return 0, json.dumps({"audit": {"model": "gpt-5.6-luna", "provider": "openai-codex", "tool_schema_count": 0}, "assistant_id": 4,
                              "decision": self.reply or {"event_id": event_id, "decision": "quiet", "message": ""}}), ""
...
class AutonomyIntegrationTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.clock = [1000.0]
        self.helper = Path(self.directory.name) / "helper"; self.helper.touch()
    ...
    async def asyncTearDown(self):
        await self.loop.close(); self.directory.cleanup()

    def begin_event(self, event_id="event-1"):
        self.loop.update_context(APPROVED, self.clock[0])
        event = {"status": "running", "task_revision": self.chat.meta["tasks"]["revision"], "human_epoch": 0,
        ...
        payload = {"session_id": "fixture-session", "event_id": event_id, "task_state": public_state(self.chat.meta["tasks"]), "human_epoch": 0, "observation": APPROVED}
    ...
    async def test_quiet_association_is_saved_without_visible_delivery(self):
        ...
        self.chat.reply = {"event_id": "event-1", "decision": "quiet", "message": "", "related_task_ids": ["task-one"]}
    ...
    async def test_invalid_related_ids_reject_inference(self):
        event, payload = self.begin_event()
        self.chat.reply = {"event_id": "event-1", "decision": "quiet", "message": "", "related_task_ids": ["not-a-task"]}
        await self.loop.decide(payload)
        session = self.loop.ledger.snapshot(self.clock[0])["recent_sessions"][0]
        self.assertEqual(event["status"], "quiet")
    ...
    async def test_real_preview_callback_writes_then_completes_only_for_current_event(self):
        ...
        self.chat.reply = {"event_id": "event-1", "decision": "check_in", "message": "Still going?", "related_task_ids": []}
        await self.loop.decide(payload)
        self.assertEqual(event["status"], "delivered")
        self.assertEqual(self.loop.stream, {"id": "event-1", "event_id": "event-1", "text": "Still going?", "status": "complete", "message_id": "4"})
    ...
    async def test_pause_source_or_task_change_interrupts_preview_and_prevents_delivery(self):
        for change in ("pause", "source", "task"):
            ...
            self.chat.emit_preview = True
            self.chat.reply = {"event_id": change, "decision": "check_in", "message": "Still going?", "related_task_ids": []}
            original = self.chat.command
            async def command(args, **kwargs):
                callback = kwargs.get("on_preview")
                if callback: callback("Writing")
                if change == "pause": self.loop.stop("paused", "test")
                elif change == "source": self.loop.update_context(OTHER, self.clock[0])
                else: self.chat.meta["tasks"]["revision"] += 1
...
class CommitMetaLedgerTests(unittest.IsolatedAsyncioTestCase):
    async def test_commit_meta_rebind_preserves_activity_journal_after_task_commit(self):
        ...
        try:
            ...
            candidate["tasks"], _ = apply_operations(candidate["tasks"], [{"op": "add", "temp_id": "new_one", "title": "Read", "due_text": None, "target_count": None, "unit": None}], based_on_revision=candidate["tasks"]["revision"], request_id="commit", source={"kind": "human"}, now=2, id_factory=lambda: "task-one")
            ...
            await chat.close(); directory.cleanup()
