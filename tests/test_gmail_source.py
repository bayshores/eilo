...
class GmailSourceTests(unittest.TestCase):
    ...
    def test_search_is_one_bounded_page_with_no_completion_claim(self):
        request = RecordedRequest([{"threads": [{"id": "a"}, {"id": "a"}, {"id": "b"}], "nextPageToken": "next"}])
        result = self.run_async(search_threads(request, "from:example@example.com", max_threads=2, page_token="prior"))
        self.assertEqual(result, {"thread_ids": ["a", "b"], "nextPageToken": "next", "has_more": True})
        ...
        payload = {
            ...
            "messages": [{
                ...
                "payload": {"mimeType": "multipart/alternative", "headers": [
                    {"name": "Subject", "value": "Hello\n world"}, {"name": "From", "value": "Sender <s@example.com>"}, {"name": "Date", "value": "Tue"},
                ], "parts": [
                    {"mimeType": "text/html", "body": {"data": encoded("<p>HTML <b>copy</b></p><script>ignore()</script>")}},
                    {"mimeType": "text/plain", "body": {"data": encoded("Plain body")}},
                    {"mimeType": "application/pdf", "body": {"attachmentId": "never-fetched"}},
                ]},
            }],
        }
    ...
    def test_read_html_nested_parts_and_all_truncation_signals(self):
        payload = {"id": "t", "messages": [
            {"id": "m1", "threadId": "t", "payload": {"parts": [{"parts": [{"mimeType": "text/html", "body": {"data": encoded("<div>Hello</div><style>hide</style><p>there</p>")}}]}]}},
            {"id": "m2", "threadId": "t", "payload": {"mimeType": "text/plain", "body": {"data": encoded("second")}}},
        ]}
        result = self.run_async(read_thread(RecordedRequest([payload]), "t", max_messages=1, max_chars=5))
    ...
    def test_thread_url_is_quoted_and_invalid_or_mismatched_responses_fail_safely(self):
        ...
        with self.assertRaisesRegex(GmailSourceError, "incomplete"):
            self.run_async(read_thread(RecordedRequest([{"id": "other", "messages": []}]), "wanted"))
        with self.assertRaises(GmailSourceError):
    ...
    def test_malformed_mime_data_is_omitted_and_marked_truncated(self):
        payload = {"id": "t", "messages": [{"id": "m", "threadId": "t", "payload": {
    ...
    def test_transport_details_do_not_escape(self):
        ...
        self.assertEqual(str(caught.exception), "Gmail could not be read. Please try reconnecting it.")
