...
function createBackendSupervisor(options) {
  ...
  if (!Number.isInteger(attempts) || attempts < 1) throw new TypeError("attempts must be positive.");
  if (!Number.isInteger(requestTimeoutMs) || requestTimeoutMs < 1 || !Number.isInteger(shutdownTimeoutMs) || shutdownTimeoutMs < 1) throw new TypeError("timeouts must be positive.");
  ...
  function validateCheckout() {
    if (!path.isAbsolute(rootPath || "")) throw new Error("The eïlo checkout path must be absolute.");
    let actualRoot;
    try {
    ...
    if (actualRoot !== expectedRoot) throw new Error("The configured eïlo checkout is not the approved canonical checkout.");
    const script = path.join(actualRoot, "scripts", "local-chat");
    const runtime = path.join(actualRoot, ".runtime", "venv", "bin", "python");
    try {
      if (!fs.statSync(script).isFile() || !fs.statSync(runtime).isFile()) throw new Error("missing");
    } catch {
  ...
  async function inspectService(root) {
    ...
    if (!identity || identity.app !== DESKTOP_IDENTITY.app || identity.protocol !== DESKTOP_IDENTITY.protocol || identity.workspace !== root) return { kind: "foreign" };
  ...
  function watchOwnedChild(process) {
    process.once("error", (error) => {
      ...
      child = null;
      report("failed", error instanceof Error ? error : new Error("The local service could not start."));
    });
  ...
  async function stopOwnedChild() {
    ...
    if (result.type === "error") {
      const error = result.error instanceof Error ? result.error : new Error("The owned local service stopped with an error.");
      report("failed", error);
      throw error;
    }
    report("stopped");
  ...
  function stop() {
    ...
    const operation = (async () => {
      ...
      if (startPromise) {
        try { await startPromise; } catch { /* startup failure already reported */ }
      }
      await stopOwnedChild();
    })();
  ...
  return Object.freeze({
    start,
    stop,
    status: () => ({ phase, error: lastError ? lastError.message : null, ownsChild: Boolean(child), url: urlFor("/") }),
    validateCheckout,
  });
}
