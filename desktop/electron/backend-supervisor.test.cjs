...
function serviceFetch({ available = true, root = ROOT } = {}) {
  return async (url, options) => {
    ...
    if (url.endsWith("/api/desktop")) return response(true, { app: "eilo", protocol: 1, workspace: root });
    if (url.endsWith("/api/state")) return response(true, { messages: ["private state is never retained"] });
...
test("attaches only to the exact desktop identity and never stops it", async () => {
  ...
  const foreign = createBackendSupervisor({
    rootPath: ROOT, canonicalRoot: ROOT, fs: checkoutFs(),
    fetch: serviceFetch({ root: "/another/eilo" }),
    spawn: () => { spawns += 1; return child(); }, attempts: 1,
  });
  ...
  const attached = createBackendSupervisor({ rootPath: ROOT, canonicalRoot: ROOT, fs: checkoutFs(), fetch: serviceFetch(), spawn: () => { throw new Error("must not spawn"); } });
...
test("bounds every probe with the injected request timeout", async () => {
  ...
  const supervisor = createBackendSupervisor({
    rootPath: ROOT, canonicalRoot: ROOT, fs: checkoutFs(), fetch: serviceFetch(),
    abortSignalTimeout: (milliseconds) => { timeoutCalls.push(milliseconds); return { milliseconds }; },
    requestTimeoutMs: 37, spawn: () => { throw new Error("must not spawn"); },
  });
...
test("malformed identity and request timeouts fail closed without spawning", async () => {
  for (const fetch of [
    ...
    let spawns = 0;
    const supervisor = createBackendSupervisor({
      rootPath: ROOT, canonicalRoot: ROOT, fs: checkoutFs(), fetch,
      spawn: () => { spawns += 1; return child(); }, attempts: 1,
    });
...
test("starts the approved launcher with an allowlisted environment", async () => {
  ...
  const calls = [];
  const supervisor = createBackendSupervisor({
    rootPath: ROOT, canonicalRoot: ROOT, fs: checkoutFs(), fetch: serviceFetch({ available: false }),
    spawn: (...args) => { calls.push(args); return spawned; }, attempts: 1,
    environment: { PATH: "/safe/bin", HOME: "/safe/home", TOKEN: "do-not-pass", EILO_SECRET: "do-not-pass" },
  });
  await assert.rejects(supervisor.start(), /did not become ready/);
  assert.deepEqual(calls[0], [
    "/approved/eilo/scripts/local-chat", ["start", "--port", "8765"],
...
test("failed readiness cleans up the owned child and reports failure", async () => {
  ...
  const states = [];
  const supervisor = createBackendSupervisor({
    rootPath: ROOT, canonicalRoot: ROOT, fs: checkoutFs(), fetch: serviceFetch({ available: false }), spawn: () => spawned,
    onState: (state) => states.push(state.phase), attempts: 2, sleep: async () => {},
  });
  await assert.rejects(supervisor.start(), /did not become ready/);
  assert.deepEqual(spawned.killCalls, ["SIGTERM"]);
...
test("stop waits for an owned child exit", async () => {
  ...
  const ready = createBackendSupervisor({
    rootPath: ROOT, canonicalRoot: ROOT, fs: checkoutFs(), spawn: () => owned,
    fetch: async (url, options) => {
      calls += 1;
      assert.equal(options.headers["X-Eilo-Client"], "local-chat");
      if (calls <= 2) throw Object.assign(new Error("not listening yet"), { cause: { code: "ECONNREFUSED" } });
      return url.endsWith("/api/desktop") ? response(true, { app: "eilo", protocol: 1, workspace: ROOT }) : response(true, {});
...
test("an owned child error is handled without an unhandled emitter error", async () => {
  ...
  const supervisor = createBackendSupervisor({
    ...
    fetch: async (url) => {
      requests += 1;
      if (requests <= 2) throw Object.assign(new Error("not listening yet"), { cause: { code: "ECONNREFUSED" } });
      return url.endsWith("/api/desktop") ? response(true, { app: "eilo", protocol: 1, workspace: ROOT }) : response(true, {});
  ...
  assert.deepEqual(supervisor.status(), {
    phase: "failed", error: "launch pipe failed", ownsChild: false, url: "http://127.0.0.1:8765/",
  });
});
...
test("shutdown times out, escalates only the owned child, and reports failure", async () => {
  ...
  owned.killCalls = [];
  owned.kill = (signal) => { owned.killCalls.push(signal); return true; };
  let requests = 0;
  const supervisor = createBackendSupervisor({
    rootPath: ROOT, canonicalRoot: ROOT, fs: checkoutFs(), spawn: () => owned,
    fetch: async (url) => {
    ...
    setTimeoutFn: (callback) => { queueMicrotask(callback); return {}; }, clearTimeoutFn: () => {}, shutdownTimeoutMs: 1,
  });
  await supervisor.start();
  ...
  assert.deepEqual(owned.killCalls, ["SIGTERM", "SIGKILL"], "retry never signals or spawns a retained child");
});
...
test("concurrent starts share one startup operation and spawn one child", async () => {
  ...
  const supervisor = createBackendSupervisor({
    rootPath: ROOT, canonicalRoot: ROOT, fs: checkoutFs(),
    spawn: () => { spawns += 1; return owned; },
    fetch: async (url) => {
      requests += 1;
      if (requests === 1) throw Object.assign(new Error("not listening yet"), { cause: { code: "ECONNREFUSED" } });
      return url.endsWith("/api/desktop") ? response(true, { app: "eilo", protocol: 1, workspace: ROOT }) : response(true, {});
    }, attempts: 2,
  });
...
test("stop during a pending startup waits for and stops the one owned child", async () => {
  const owned = child();
  let releaseProbe;
  const firstProbe = new Promise((resolve, reject) => { releaseProbe = { resolve, reject }; });
  let requests = 0, spawns = 0;
  const supervisor = createBackendSupervisor({
    rootPath: ROOT, canonicalRoot: ROOT, fs: checkoutFs(),
    spawn: () => { spawns += 1; return owned; },
  ...
  releaseProbe.reject(Object.assign(new Error("not listening yet"), { cause: { code: "ECONNREFUSED" } }));
